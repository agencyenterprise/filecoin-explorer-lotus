 ...
 */
;(function() {
  function _dc_graph(d3, crossfilter, dc) {
    'use strict'
    ...
    var property = function(defaultValue, unwrap) {
      if (unwrap === undefined) unwrap = get_original
      ...
      ret._eval = function(o, n) {
        if (n === 0 || !cascade.length) return dc_graph.functor_wrap(ret(), unwrap)(o)
        else {
    ...
    function named_children() {
      ...
      var f = function(id, object) {
        if (arguments.length === 1) return _children[id]
        // do not notify unnecessarily
        if (_children[id] === object) return this
        if (_children[id]) _children[id].parent(null)
        _children[id] = object
    ...
    function multiply_properties(pred, props, blend) {
      var props2 = {}
      for (var p in props) props2[p] = blend(pred, param(props[p]))
      return props2
    }
    ...
    function node_edge_conditions(npred, epred, props) {
      var nprops = {},
        ...
        badprops = []
      for (var p in props) {
        if (/^node/.test(p)) nprops[p] = props[p]
        else if (/^edge/.test(p)) eprops[p] = props[p]
      ...
      if (epred) Object.assign(props2, conditional_properties(epred, eprops))
      return props2
    }
    ...
    function cascade(parent) {
      return function(level, add, props) {
        for (var p in props) {
          if (!parent[p]) throw new Error('unknown attribute ' + p)
    ...
    function generate_path(pts, bezDegree, close) {
      var cats = ['M', pts[0].x, ',', pts[0].y],
        remain = bezDegree
      var hasNaN = false
      for (var i = 1; i < pts.length; ++i) {
        if (isNaN(pts[i].x) || isNaN(pts[i].y)) hasNaN = true
        cats.push(remain === bezDegree ? bez_cmds[bezDegree] : ' ', pts[i].x, ',', pts[i].y)
        if (--remain === 0) remain = bezDegree
      }
      if (remain != bezDegree) console.log("warning: pts.length didn't match bezian degree", pts, bezDegree)
      if (close) cats.push('Z')
      return cats.join('')
    }
    ...
    dc_graph.event_coords = function(diagram) {
      ...
      return diagram.invertCoord([d3.event.clientX - bound.left, d3.event.clientY - bound.top])
    }
    ...
    function ancestor_has_class(element, classname) {
      if (d3.select(element).classed(classname)) return true
    ...
    dc_graph.depth_first_traversal = function(callbacks) {
      // {[init, root, row, tree, place, sib, push, pop, skip,] finish, nodeid, sourceid, targetid}
      return function(nodes, edges) {
        callbacks.init && callbacks.init()
        if (callbacks.tree)
          edges = edges.filter(function(e) {
            return callbacks.tree(e)
          })
        var indegree = {}
        ...
        function place_tree(n, r) {
          ...
          if (outmap[key])
            outmap[key].forEach(function(e, ei) {
              ...
              if (ei && callbacks.sib) callbacks.sib(false, nmap[callbacks.targetid(outmap[key][ei - 1])], target)
        ...
        var roots
        if (callbacks.root)
          roots = nodes.filter(function(n) {
            return callbacks.root(n)
          })
        else {
          roots = nodes.filter(function(n) {
            return !indegree[callbacks.nodeid(n)]
          })
          if (nodes.length && !roots.length)
            // all nodes are in a cycle
            roots = [nodes[0]]
        }
        roots.forEach(function(n, ni) {
          if (ni && callbacks.sib) callbacks.sib(true, roots[ni - 1], n)
          callbacks.push && callbacks.push()
    ...
    dc_graph.undirected_dfs = function(callbacks) {
      // {[comp, node], nodeid, sourceid, targetid}
      return function(nodes, edges) {
    ...
    function regenerate_objects(preserved, list, need, key, assign, create, destroy) {
      ...
      function wrap(o) {
        var k = key(o)
        if (!preserved[k]) create(k, (preserved[k] = {}), o)
        var o1 = preserved[k]
        assign(o1, o)
        keep[k] = true
        return o1
      ...
      if (need)
        need.forEach(function(k) {
          if (!preserved[k]) {
            // hasn't been created, needs to be
            create(k, (preserved[k] = {}), null)
            assign(preserved[k], null)
          }
          if (!keep[k]) {
            // wasn't in list, should be
    ...
    function point_on_polygon(points, x0, y0, x1, y1) {
      for (var i = 0; i < points.length; ++i) {
        ...
        var isect = segment_intersection(points[i].x, points[i].y, points[next].x, points[next].y, x0, y0, x1, y1)
    ...
    function normalize_shape_def(diagram, n) {
      ...
      if (!def) return default_shape
      if (typeof def === 'string') return { shape: def }
      return def
    }
    ...
    function shape_changed(diagram) {
      return function(n) {
        ...
        if (def.shape !== old.shape) return true
        else if (def.shape === 'polygon') {
          return (
            def.shape.sides !== old.sides ||
            def.shape.skew !== old.skew ||
            def.shape.distortion !== old.distortion ||
            def.shape.rotation !== old.rotation
          )
        } else return false
      }
    }
    ...
    function node_label_padding(diagram, n) {
      ...
      if (typeof nlp === 'number' || typeof nlp === 'string') return { x: +nlp, y: +nlp }
      else return nlp
    }
    ...
    function polygon_attrs(diagram, n) {
      return {
        d: function(n) {
          ...
          var yext = d3.extent(angles, function(theta) {
            return theta.y
          })
          if (def.regular) rx = ry = Math.max(rx, ry)
          else if (rx < ry && !def.squeeze) rx = ry
          else ry = ry / Math.min(-yext[0], yext[1])
    ...
    function binary_search(f, a, b) {
      var patience = 100
      if (f(a).val >= 0) throw new Error('f(a) must be less than 0')
      if (f(b).val <= 0) throw new Error('f(b) must be greater than 0')
      while (true) {
        if (!--patience) throw new Error('patience ran out')
        var c = (a + b) / 2,
          f_c = f(c),
          fv = f_c.val
        if (Math.abs(fv) < 0.5) return f_c
        if (fv > 0) b = c
        else a = c
      }
    }

    function draw_edge_to_shapes(diagram, e, sx, sy, tx, ty, neighbor, dir, offset, source_padding, target_padding) {
      var deltaX, deltaY, sp, tp, points, bezDegree, headAng, retPath
      if (!neighbor) {
        sp = e.sourcePort.pos
        tp = e.targetPort.pos
        if (!sp) sp = { x: 0, y: 0 }
    ...
    function is_one_segment(path) {
      return (path.bezDegree === 1 && path.points.length === 2) || (path.bezDegree === 3 && path.points.length === 4)
    }
    ...
    function chop_bezier(points, end, dist) {
      ...
      do {
        ...
        d2 = dx * dx + dy * dy
        dt /= 2
        if (d2 > dist2) t -= dt * dir
        else t += dt * dir
        //console.log('dist', dist, 'dir', dir, 'd', d, 't', t, 'dt', dt);
      } while (dt > 0.0000001 && Math.abs(d2 - dist2) > EPS)
      points = points.slice()
      if (end === 'head') return points.slice(0, points.length - 4).concat(parts[0])
      else return parts[1].concat(points.slice(4))
    }
    ...
    dc_graph.ellipse_shape = function() {
      var _shape = {
        ...
        create: function(nodeEnter) {
          nodeEnter.insert('ellipse', ':first-child').attr('class', 'node-shape')
        },
        update: function(node) {
          node.select('ellipse.node-shape').attr(ellipse_attrs(_shape.parent()))
        },
      }
      return _shape
    }
    ...
    dc_graph.polygon_shape = function() {
      var _shape = {
        ...
        create: function(nodeEnter) {
          nodeEnter.insert('path', ':first-child').attr('class', 'node-shape')
        },
        update: function(node) {
          node.select('path.node-shape').attr(polygon_attrs(_shape.parent()))
        },
      }
      return _shape
    }
    ...
    dc_graph.rounded_rectangle_shape = function() {
      var _shape = {
        ...
        update: function(node) {
          node.select('rect.node-shape').attr({
            x: function(n) {
              return -n.dcg_rx
            },
            y: function(n) {
    ...
    dc_graph.elaborated_rectangle_shape = function() {
      ...
      _shape.update = function(node) {
        node.select('path.node-shape').attr('d', function(n) {
      ...
      curve: function(open, side) {
        return {
          ...
          drawFunction: function(marker, ofs, stemWidth) {
            ...
            if (side === 'left') instrs.push('v', -stemWidth / 2)
            instrs.push('A', 3.5, 3.5, 0, 0, 0, (side === 'right' ? 7.5 : 4) + ofs[0], side === 'right' ? 0 : -3.5)
            if (side === 'right') instrs.push('v', -stemWidth / 2)
            marker.append('svg:path').attr({
              d: instrs.join(' '),
              'stroke-width': 1,
              fill: 'none',
      ...
      icurve: function(open, side) {
        return {
          ...
          drawFunction: function(marker, ofs, stemWidth) {
            ...
            if (side === 'left') instrs.push('v', -stemWidth / 2)
            instrs.push('A', 3.5, 3.5, 0, 0, 1, (side === 'right' ? 0.5 : 4) + ofs[0], side === 'right' ? 0 : -3.5)
            if (side === 'right') instrs.push('v', -stemWidth / 2)
            marker.append('svg:path').attr({
              d: instrs.join(' '),
              'stroke-width': 1,
        ...
        if (!open)
          return {
            ...
            kernstems: function(stemWidth) {
              return [side ? 0 : 0.75 * stemWidth, side ? 0 : 0.75 * stemWidth]
            },
            drawFunction: function(marker, ofs, stemWidth) {
              var upoints = [{ x: 0, y: 0 }]
              if (side !== 'left') upoints.push({ x: 6, y: 4 })
              else upoints.push({ x: 6, y: -4 })
              upoints.push({ x: 12, y: 0 })
              if (!side) upoints.push({ x: 6, y: -4 })
          ...
          return {
            ...
            drawFunction: function(marker, ofs, stemWidth) {
              ...
              if (side !== 'left') upoints.push({ x: 6, y: 3.4 })
              else upoints.push({ x: 6, y: -3.4 })
              upoints.push({ x: 11.1, y: 0 })
              if (!side) upoints.push({ x: 6, y: -3.4 })
              var points = upoints.map(offsetx(ofs[0]))
              marker.append('svg:path').attr({
        ...
        if (!open)
          return {
            ...
            drawFunction: function(marker, ofs, stemWidth) {
              if (side) {
                marker.append('svg:path').attr({
                  d: ['M', ofs[0], 0, 'A', 4, 4, 0, 0, side === 'left' ? 1 : 0, 8 + ofs[0], 0].join(' '),
                  'stroke-width': 0,
                })
                marker.append('svg:path').attr({
                  d: ['M', ofs[0], 0, 'h 8'].join(' '),
          ...
          return {
            ...
            drawFunction: function(marker, ofs, stemWidth) {
              if (side) {
                marker.append('svg:path').attr({
                  d: ['M', 0.5 + ofs[0], 0, 'A', 3.5, 3.5, 0, 0, side === 'left' ? 1 : 0, 7.5 + ofs[0], 0].join(' '),
                  'stroke-width': 1,
                  fill: 'none',
                })
                marker.append('svg:path').attr({
      ...
      normal: function(open, side) {
        if (!open)
          return {
            ...
            drawFunction: function(marker, ofs, stemWidth) {
              var upoints = []
              if (side === 'left') upoints.push({ x: 0, y: 0 })
              else upoints.push({ x: 0, y: 3 })
              switch (side) {
                case 'left':
              ...
              if (side === 'right') upoints.push({ x: 0, y: 0 })
              else upoints.push({ x: 0, y: -3 })
              var points = upoints.map(offsetx(ofs[0]))
              marker
                .append('svg:path')
              ...
              if (side) {
                marker.append('svg:path').attr({
                  d: ['M', ofs[0], 0, 'h', 8 - (4 * stemWidth) / 3].join(' '),
                  'stroke-width': stemWidth,
                  fill: 'none',
                })
              }
      ...
      inv: function(open, side) {
        if (!open)
          return {
            ...
            drawFunction: function(marker, ofs, stemWidth) {
              var upoints = []
              if (side === 'left') upoints.push({ x: 8, y: 0 })
              else upoints.push({ x: 8, y: 3 })
              switch (side) {
                case 'left':
              ...
              if (side === 'right') upoints.push({ x: 8, y: 0 })
              else upoints.push({ x: 8, y: -3 })
              var points = upoints.map(offsetx(ofs[0]))
              marker
                .append('svg:path')
              ...
              if (side) {
                marker.append('svg:path').attr({
                  d: ['M', (4 * stemWidth) / 3 + ofs[0], 0, 'h', 8 - (4 * stemWidth) / 3].join(' '),
                  'stroke-width': stemWidth,
                  fill: 'none',
                })
              }
          ...
          return {
            ...
            drawFunction: function(marker, ofs, stemWidth) {
              ...
              if (side) {
                marker.append('svg:path').attr({
                  d: ['M', (4 * stemWidth) / 3 + ofs[0], 0, 'h', 8 - 4 / 3].join(' '),
                  'stroke-width': stemWidth,
                  fill: 'none',
                })
              }
      ...
      vee: function(open, side) {
        return {
          ...
          drawFunction: function(marker, ofs, stemWidth) {
            ...
            if (side === 'right') upoints.splice(0, 1, { x: 5, y: -stemWidth / 2 }, { x: 10, y: -stemWidth / 2 })
            else if (side === 'left') upoints.splice(2, 1, { x: 10, y: stemWidth / 2 }, { x: 5, y: stemWidth / 2 })
            var points = upoints.map(offsetx(ofs[0]))
            marker
              .append('svg:path')
              .attr('d', generate_path(points, 1, true))
      ...
      crow: function(open, side) {
        return {
          ...
          drawFunction: function(marker, ofs, stemWidth) {
            ...
            if (side === 'right') upoints.splice(0, 1, { x: 5, y: -stemWidth / 2 }, { x: 0, y: -stemWidth / 2 })
            else if (side === 'left') upoints.splice(2, 1, { x: 0, y: stemWidth / 2 }, { x: 5, y: stemWidth / 2 })
            var points = upoints.map(offsetx(ofs[0]))
            marker
              .append('svg:path')
              .attr('d', generate_path(points, 1, true))
    ...
    function arrow_offsets(parts, stemWidth) {
      ...
      return parts.map(function(p, i) {
        ...
        if (p.kernstems) {
          ...
          if (typeof kernstems === 'function') kernstems = kernstems(stemWidth)
          if (i !== 0 && kernstems[1]) {
            var last = parts[i - 1]
            if (last.stems && last.stems[0]) fr[0] -= kernstems[1]
          }
          if (kernstems[0]) {
            var kern = false
            if (i === parts.length - 1) kern = true
            else {
              var next = parts[i + 1]
              if (next.stems && next.stems[1]) kern = true
            }
            if (kern) br[0] += kernstems[0]
          }
        }
        if (i === 0) {
          frontRef = fr
    ...
    function arrow_bounds(parts, stemWidth) {
      ...
      parts.forEach(function(p, i) {
        ...
        if (!viewBox) viewBox = vb.slice()
        else viewBox = union_viewbox(viewBox, [vb[0] + ofs[0], vb[1] + ofs[1], vb[2], vb[3]])
      })
    ...
    function edgeArrow(diagram, arrdefs, e, kind, desc) {
      ...
      if (desc) {
        ...
        if (e[kind + 'ArrowLast'] === arrow_sig()) return id
      }
      ...
      if (parts.length) {
        ...
        marker.html(null)
        parts.forEach(function(p, i) {
          marker.call(p.drawFunction, add_points([-strokeOfs / arrowSize, 0], bounds.offsets[i].offset), stemWidth)
        })
      }
    ...
    dc_graph.text_contents = function() {
      var _contents = {
        ...
        update: function(container) {
          var text = container.selectAll('text.node-label').data(function(n) {
            return [n]
          })
          text
            .enter()
          ...
          var tspan = text.selectAll('tspan').data(function(n) {
            ...
            if (!lines) return []
            else if (typeof lines === 'string') lines = [lines]
            ...
            if (is_ie() || is_safari()) first += 0.3
            return lines.map(function(line, i) {
              return { node: n, line: line, yofs: (i == 0 ? first : lineHeight) + 'em' }
            })
          })
          ...
          tspan.exit().remove()
          text.attr('fill', _contents.parent().nodeLabelFill.eval)
        },
    ...
    dc_graph.with_icon_contents = function(contents, width, height) {
      var _contents = {
        ...
        update: function(container) {
          var g = container.selectAll('g.with-icon').data(function(n) {
            return [n]
          })
          var gEnter = g.enter()
          gEnter
          ...
          contents.selectContent(g).attr('transform', 'translate(' + width / 2 + ')')
    ...
    dc_graph.diagram = function(parent, chartGroup) {
      ...
      var _nodes = {},
        _edges = {} // hold state between runs
      ...
      function deprecate_layout_algo_parameter(name) {
        return function(value) {
          if (!_diagram.layoutEngine()) _diagram.layoutAlgorithm('cola', true)
          var engine = _diagram.layoutEngine()
          if (engine.getEngine) engine = engine.getEngine()
          if (engine[name]) {
            console.warn(
              'property is deprecated, call on layout engine instead: dc_graph.diagram.%c' + name,
              'font-weight: bold',
            )
            if (!arguments.length) return engine[name]()
            engine[name](value)
          } else {
            console.warn(
              'property is deprecated, and is not supported for Warning: dc_graph.diagram.<b>' +
                name +
                '</b> is deprecated, and it is not supported for the "' +
                engine.layoutAlgorithm() +
                '" layout algorithm: ignored.',
            )
      ...
      _diagram.height = function(height) {
        ...
        if (dc.utils.isNumber(height) || !height || height === 'auto') _height = height
      ...
      _diagram.width = function(width) {
        ...
        if (dc.utils.isNumber(width) || !width || width === 'auto') _width = width
      ...
      _diagram.root = property(null).react(function(e) {
        if (e.empty()) console.log('Warning: parent selector ' + parent + " doesn't seem to exist")
      })
      ...
      _diagram.zoomExtent = property([0.1, 2])
      ...
      _diagram.zoomToFit = function(animate) {
        if (!(_nodeLayer && _edgeLayer)) return
      ...
      _diagram.initialOnly = deprecated_property(
     ...
     **/
      ...
      _diagram.legend = deprecate_function(
        '.legend() is deprecated; use .child() for more control & multiple legends',
        function(_) {
          if (!arguments.length) return _diagram.child('node-legend')
      ...
      _diagram.layoutAlgorithm = function(value, skipWarning) {
        if (!arguments.length) return _diagram.layoutEngine() ? _diagram.layoutEngine().layoutAlgorithm() : 'cola'
        if (!skipWarning)
          console.warn(
            'dc.graph.diagram.layoutAlgorithm is depecrated - pass the layout engine object to dc_graph.diagram.layoutEngine instead',
          )
      ...
      _diagram.layoutEngine = property(null).react(function(val) {
        if (val && val.parent) val.parent(_diagram)
        if (_g) {
          // already rendered
      ...
      _diagram.forEachShape = function(node, f) {
        _diagram.forEachChild(
          node,
          _diagram.shape,
          function(n) {
            return n.dcg_shape.shape
          },
          f,
        )
      }
      ...
      _diagram.renderNode = _diagram._enterNode = function(nodeEnter) {
        if (_diagram.nodeTitle()) nodeEnter.append('title')
      ...
      _diagram.redrawNode = _diagram._updateNode = function(node) {
        ...
        node.select('title').text(_diagram.nodeTitle.eval)
      ...
      _diagram.redrawEdge = _diagram._updateEdge = function(edge, edgeArrows) {
        ...
        edgeArrows
          ...
          .each(function(e) {
            ...
            d3.selectAll('#' + _diagram.arrowId(e, 'head')).attr('fill', _diagram.edgeStroke.eval(e))
            d3.selectAll('#' + _diagram.arrowId(e, 'tail')).attr('fill', _diagram.edgeStroke.eval(e))
          })
      }
      ...
      _diagram.stagedDelay = function(is_enter) {
        return _diagram.stageTransitions() === 'none' || (_diagram.stageTransitions() === 'modins') === !is_enter
          ? 0
          : _diagram.transitionDuration() / 2
      }
      ...
      _diagram.selectAllNodes = function(selector) {
        ...
        return (
          (_nodeLayer &&
            _nodeLayer.selectAll(selector).filter(function(n) {
              return !n.deleted
            })) ||
      ...
      _diagram.selectAllEdges = function(selector) {
        ...
        return (
          (_edgeLayer &&
            _edgeLayer.selectAll(selector).filter(function(e) {
              return !e.deleted
            })) ||
          d3.selectAll('.foo-this-does-not-exist')
        )
      ...
      _diagram.selectAllDefs = function(selector) {
        return (
          (_defs &&
            _defs.selectAll(selector).filter(function(def) {
              return !def.deleted
            })) ||
          d3.selectAll('.foo-this-does-not-exist')
        )
      ...
      function detect_size_change() {
        ...
        var newWidth = _diagram.width(),
          newHeight = _diagram.height()
        if (oldWidth !== newWidth || oldHeight !== newHeight) {
          var scale = _zoom.scale(),
            translate = _zoom.translate()
          _zoom.scale(1).translate([0, 0])
          var xDomain = _diagram.x().domain(),
            yDomain = _diagram.y().domain()
          _diagram
            .x()
      ...
      _diagram.startLayout = function() {
        ...
        if (_width === 'auto' || _height === 'auto') detect_size_change()
        else _diagram.resizeSvg()

        if (_diagram.initLayoutOnRedraw()) initLayout()
        ...
        wports = wports.filter(function(p) {
          return p.node
        })
        ...
        wedges.forEach(function(e) {
          ...
          if (name) _ports[port_name(_diagram.nodeKey.eval(e.source), null, name)].edges.push(e)
          name = _diagram.edgeTargetPortName.eval(e)
          if (name) _ports[port_name(_diagram.nodeKey.eval(e.target), null, name)].edges.push(e)
        })
        ...
        if (_diagram.induceNodes()) {
          ...
          wnodes = wnodes.filter(function(n) {
            return keeps[n.cola.dcg_nodeKey]
          })
          for (var k in _nodes) if (!keeps[k]) delete _nodes[k]
        }
        ...
        wnodes.forEach(function(v) {
          if (_diagram.nodeFixed()) v.cola.dcg_nodeFixed = _diagram.nodeFixed.eval(v)
        })

        // annotate parallel edges so we can draw them specially
        if (_diagram.parallelEdgeOffset()) {
          var em = new Array(wnodes.length)
          for (var i = 0; i < wnodes.length; ++i) em[i] = new Array(i)
          wedges.forEach(function(e) {
            e.pos = e.pos || {}
        ...
        var edge = _edgeLayer.selectAll('.edge').data(wedges, _diagram.edgeKey.eval)
        var edgeEnter = edge
          .enter()
          .append('svg:path')
          .attr({
        ...
        var edgeArrows = _edgeLayer.selectAll('.edge-arrows').data(wedges, _diagram.edgeKey.eval)
        var edgeArrowsEnter = edgeArrows
          .enter()
          .append('svg:path')
          .attr({
            class: 'edge-arrows',
        ...
        if (_diagram.edgeSort()) {
          edge.sort(function(a, b) {
            var as = _diagram.edgeSort.eval(a),
              bs = _diagram.edgeSort.eval(b)
            return as < bs ? -1 : bs < as ? 1 : 0
          })
        }

        // another wider copy of the edge just for hover events
        var edgeHover = _edgeLayer.selectAll('.edge-hover').data(wedges, _diagram.edgeKey.eval)
        var edgeHoverEnter = edgeHover
          .enter()
          .append('svg:path')
          ...
          .on('mouseover.diagram', function(e) {
            d3.select('#' + _diagram.edgeId(e) + '-label').attr('visibility', 'visible')
          })
          .on('mouseout.diagram', function(e) {
            d3.select('#' + _diagram.edgeId(e) + '-label').attr('visibility', 'hidden')
          })
        edgeHover.exit().remove()

        var edgeLabels = _edgeLayer.selectAll('g.edge-label-wrapper').data(wedges, _diagram.edgeKey.eval)
        var edgeLabelsEnter = edgeLabels
          .enter()
          .append('g')
        ...
        var textPaths = _defs.selectAll('path.edge-label-path').data(wedges, _diagram.textpathId)
        var textPathsEnter = textPaths
          .enter()
          .append('svg:path')
          .attr({
        ...
        var node = _nodeLayer.selectAll('.node').data(wnodes, _diagram.nodeKey.eval)
        var nodeEnter = node
          .enter()
          .append('g')
          .attr('class', 'node')
        ...
        if (_diagram.initialLayout()) _diagram.initialLayout()(_diagram, wnodes, wedges)

        // no layout if the topology and layout parameters haven't changed
        var skip_layout = false
        if (!_diagram.layoutUnchanged()) {
          var nodes_snapshot = JSON.stringify(
            wnodes.map(function(n) {
              return { orig: get_original(n), cola: { dcg_nodeFixed: n.cola.dcg_nodeFixed } }
            }),
          )
          var edges_snapshot = JSON.stringify(
            wedges.map(function(e) {
              return { orig: get_original(e), cola: e.cola }
            }),
          )
          if (nodes_snapshot === _nodes_snapshot && edges_snapshot === _edges_snapshot) skip_layout = true
        ...
        circle_constraints.forEach(function(c) {
          ...
          var nindices = c.nodes.map(function(x) {
            return x.node
          })
          var namef = function(i) {
            return _diagram.nodeKey.eval(wnodes[i])
          }
          var wheel = dc_graph.wheel_edges(namef, nindices, R).map(function(e) {
            var e1 = { internal: e }
        ...
        ordered_constraints.forEach(function(c) {
          var sorted = c.nodes.map(function(n) {
            return _nodes[n]
          })
          if (c.ordering) {
        ...
        function populate_cola(rnodes, redges) {
          ...
          redges.forEach(function(re) {
            ...
            if (re.points) e.cola.points = re.points
          })
        }
        _diagram
          .layoutEngine()
          .on('tick.diagram', function(nodes, edges) {
            ...
            if (!_diagram.initialOnly()) populate_cola(nodes, edges)
        ...
        if (_diagram.initialOnly())
          ...
          _diagram.layoutEngine().data(
            ...
            wnodes.map(function(v) {
              return Object.assign({}, v.cola, v.dcg_shape)
            }),
      ...
      function edge_vec(n, e) {
        ...
        if (e.source !== n) (dy = -dy), (dx = -dx)
        if (e.parallel && e.parallel.edges.length > 1 && e.source.index > e.target.index) (dy = -dy), (dx = -dx)
        return norm([dx, dy])
      }
      ...
      function propagate_port_positions(nodes, wedges, ports) {
        ...
        for (var nid in _nodePorts) {
          ...
          _nodePorts[nid].forEach(function(p) {
            if (!p.pos) project_port(_diagram, n, p)
          })
        }
        ...
        wedges.forEach(function(e) {
          ...
          e.sourcePort.pos = name
            ? ports[port_name(_diagram.nodeKey.eval(e.source), null, name)].pos
            : ports[port_name(null, _diagram.edgeKey.eval(e), 'source')].pos
          name = _diagram.edgeTargetPortName.eval(e)
          e.targetPort.pos = name
            ? ports[port_name(_diagram.nodeKey.eval(e.target), null, name)].pos
      ...
      _diagram.refresh = function(node, edge, edgeHover, edgeLabels, textPaths) {
        if (_animating) return this // but what about changed attributes?
      ...
      _diagram.requestRefresh = function(durationOverride) {
        window.requestAnimationFrame(function() {
          ...
          _diagram.refresh()
          if (durationOverride !== undefined) _diagram.transitionDuration(transdur)
        })
      }
        ...
        edge
          .each(function(e) {
            ...
            if (_diagram.edgeArrowhead.eval(e))
              d3.select('#' + _diagram.arrowId(e, 'head')).attr('orient', function() {
                return e.pos.new.orienthead
              })
            if (_diagram.edgeArrowtail.eval(e))
              d3.select('#' + _diagram.arrowId(e, 'tail')).attr('orient', function() {
      ...
      function layout_done(happens) {
        ...
        if (_needsRedraw) {
          ...
          window.setTimeout(function() {
            if (!_diagram.isRunning())
              // someone else may already have started
              _diagram.redraw()
          }, 0)
        }
      }
        ...
        switch (_diagram.enforceEdgeDirection()) {
          case 'LR':
            if (spos.x >= tpos.x) {
              ...
              return {
                points: [first, { x: first.x + dx, y: first.y - dx / 2 }, { x: last.x - dx, y: last.y - dx / 2 }, last],
                bezDegree: 3,
          ...
          case 'TB':
            if (spos.y >= tpos.y) {
              ...
              return {
                points: [first, { x: first.x + dy / 2, y: first.y + dy }, { x: last.x + dy / 2, y: last.y - dy }, last],
                bezDegree: 3,
                sourcePort: path.sourcePort,
                targetPort: path.targetPort,
              }
      ...
      function calc_edge_path(e, age, sx, sy, tx, ty) {
        ...
        var source_padding = source.dcg_ry + _diagram.nodeStrokeWidth.eval(source) / 2,
          target_padding = target.dcg_ry + _diagram.nodeStrokeWidth.eval(target) / 2
        for (var p = 0; p < parallel.edges.length; ++p) {
          ...
          if (parallel.edges.length > 1 && parallel.rev[p]) path.points.reverse()
          if (_diagram.enforceEdgeDirection()) path = enforce_path_direction(path, source.cola, target.cola)
          var path0 = {
            points: path.points,
      ...
      function render_edge_label_path(age) {
        return function(e) {
          ...
          var points =
            path.points[path.points.length - 1].x < path.points[0].x ? path.points.slice(0).reverse() : path.points
        ...
        if (_diagram.portStyle.enum().length) {
          ...
          if (ports)
            ports.forEach(function(p) {
              ...
              if (!portStyle || !_diagram.portStyle(portStyle)) return
      ...
      function auto_zoom(animate) {
        if (_diagram.fitStrategy()) {
          if (!_bounds) return
          var vwidth = _bounds.right - _bounds.left,
          ...
          if (_diagram.DEBUG_BOUNDS) debug_bounds(_bounds)
          var fitS = _diagram.fitStrategy(),
            translate = [0, 0],
            scale = 1
          if (['default', 'vertical', 'horizontal'].indexOf(fitS) >= 0) {
            ...
            var sides = fitS
              ...
              .toLowerCase()
              .split('')
            if (sides.length > 2) throw new Error('align_ expecting 0-2 sides, not ' + sides.length)
            ...
            if (sides.includes('c')) {
              if (!horzalign) translate[0] = center_horizontally(translate, bounds)
              if (!vertalign) translate[1] = center_vertically(translate, bounds)
            }
          } else if (fitS === 'zoom') {
            scale = _zoom.scale()
      ...
      function unsurprising_orient(oldorient, neworient) {
        ...
        if (Math.abs(oldang - newang) > Math.PI) {
          if (newang > oldang) oldang += 2 * Math.PI
      ...
      function draw(
        ...
        edge.each(function(e) {
          if (e.cola.points) {
            ...
            e.pos.new = {
              ...
              full: path0,
              orienthead:
                angle_between_points(path.points[path.points.length - 1], path0.points[path0.points.length - 1]) +
            ...
            if (!e.pos.new) calc_edge_path(e, 'new', e.source.cola.x, e.source.cola.y, e.target.cola.x, e.target.cola.y)
          }
          if (e.pos.old) {
            if (
              ...
              if (is_one_segment(e.pos.old.path)) {
                ...
                e.pos.old.path.points = split_bezier_n(
                  as_bezier3(e.pos.old.path),
        ...
        var arrowtrans = edgeArrows
          ...
          .attr('opacity', _diagram.edgeOpacity.eval)
        ;(animatePositions ? etrans : edge).attr('d', function(e) {
          var when = _diagram.stageTransitions() === 'insmod' && edgeEntered[_diagram.edgeKey.eval(e)] ? 'old' : 'new'
          return render_edge_path(when)(e)
        })
        ;(animatePositions ? arrowtrans : edgeArrows).attr('d', function(e) {
          var when = _diagram.stageTransitions() === 'insmod' && edgeEntered[_diagram.edgeKey.eval(e)] ? 'old' : 'new'
          return render_edge_path(when, true)(e)
        })
        var elabels = edgeLabels.selectAll('text').data(function(e) {
          var labels = _diagram.edgeLabel.eval(e)
          if (!labels) return []
          else if (typeof labels === 'string') return [labels]
          else return labels
        })
        elabels
          .enter()
        ...
        textPathsEnter.attr('d', render_edge_label_path(_diagram.stageTransitions() === 'modins' ? 'new' : 'old'))
        var textTrans = textPaths
          .transition()
          .duration(_diagram.stagedDuration())
          .delay(function(e) {
            return _diagram.stagedDelay(edgeEntered[_diagram.edgeKey.eval(e)])
        ...
        if (_diagram.stageTransitions() === 'insmod' && animatePositions) {
          ...
          if (_diagram.stagedDuration() >= 50) {
            ...
            etrans.attr('d', render_edge_path('new'))
            textTrans.attr('d', render_edge_path('new'))
            arrowtrans.attr('d', render_edge_path('new', true))
          }
        }
        ...
        _animating = true
        if (!_diagram.showLayoutSteps())
          endall([ntrans, etrans, textTrans], function() {
            _animating = false
            layout_done(true)
          })

        if (animatePositions) edgeHover.attr('d', render_edge_path('new'))
      ...
      _diagram.render = function() {
        if (_svg) _dispatch.reset()
        if (!_diagram.initLayoutOnRedraw()) initLayout()
        _diagram.resetSvg()
        _g = _svg.append('g').attr('class', 'draw')

        var layers = ['edge-layer', 'node-layer']
        if (_diagram.edgesInFront()) layers.reverse()
        _g.selectAll('g')
          .data(layers)
          .enter()
          .append('g')
      ...
      _diagram.on = function(event, f) {
        if (arguments.length === 1) return _dispatch.on(event)
     ...
     **/
      ...
      _diagram.addOrRemoveDef = function(id, whether, tag, onEnter) {
        ...
        var selEnter = sel
          ...
          .append(tag)
          .attr('id', id)
        if (selEnter.size() && onEnter) selEnter.call(onEnter)
        sel.exit().remove()
        return sel
      }
      ...
      function bring_in_bounds(translate) {
        ...
        if (less1 && less2)
          if (lessExt) align = 'top'
          else align = 'bottom'
        else if (!less1 && !less2)
          if (lessExt) align = 'bottom'
          else align = 'top'
        switch (align) {
          case 'top':
            translate[1] = align_top(translate, bounds.top)
      ...
      function doZoom() {
        ...
        if (_diagram.restrictPan()) _zoom.translate((translate = bring_in_bounds(d3.event.translate)))
      ...
      _diagram.resizeSvg = function(w, h) {
        if (_svg) {
          _svg
            .attr('width', w || (_width === 'auto' ? '100%' : _diagram.width()))
      ...
      function generateSvg() {
        ...
        if (_diagram.mouseZoomable()) {
          ...
          if ((mod = _diagram.modKeyZoom())) {
            if (Array.isArray(mod)) mods = mod.slice()
            else if (typeof mod === 'string') mods = [mod]
            else mods = ['Alt']
            var mouseDown = false,
              modDown = false,
              zoomEnabled = false
            _svg
              .on('mousedown.modkey-zoom', function() {
                mouseDown = true
              })
      ...
      _diagram.invertCoord = function(clientCoord) {
        return [_diagram.x().invert(clientCoord[0]), _diagram.y().invert(clientCoord[1])]
      }
    ...
    dc_graph.engines = {
      ...
      instantiate: function(layout, args, worker) {
        ...
        params.forEach(function(p) {
          if (args[p]) engine[p](args[p])
        })
        if (engine.supportsWebworker && engine.supportsWebworker() && worker) engine = dc_graph.webworker_layout(engine)
        return engine
      },
      available: function() {
      ...
      unregister: function(layoutName) {
        ...
        if (entry.name === layoutName) remove = true
        else {
          var j = entry.names.indexOf(layoutName)
          if (j >= 0) entry.names.splice(j, 1)
          else console.warn('search for engine failed', layoutName)
          if (entry.names.length === 0) remove = true
        }
        if (remove) dc_graph._engines.splice(i, 1)
        return true
      },
      register: function(entry) {
        ...
        if (entry.name) this.unregister(entry.name)
        else if (entry.names)
    ...
    function create_worker(layoutAlgorithm) {
      if (!_workers[layoutAlgorithm]) {
        ...
        worker.worker.onmessage = function(e) {
          ...
          if (!worker.layouts[layoutId]) throw new Error('layoutId "' + layoutId + '" unknown!')
    ...
    dc_graph.webworker_layout = function(layoutEngine) {
      var _tick,
        _done,
        _dispatch = d3.dispatch('init', 'start', 'tick', 'end')
      ...
      engine.parent = function(parent) {
        if (layoutEngine.parent) layoutEngine.parent(parent)
      }
      engine.init = function(options) {
        options = layoutEngine.optionNames().reduce(function(options, option) {
    ...
    dc_graph.apply_graphviz_accessors = function(diagram) {
      diagram
        .nodeLabel(function(n) {
          ...
          if (label === undefined) label = n.key
        ...
        .nodeShape(function(n) {
          return nvalue(n).shape
        })
        .nodeFill(function(n) {
          return nvalue(n).fillcolor || 'white'
        })
        ...
        .nodeLabelFill(function(n) {
          return nvalue(n).fontcolor || 'black'
        })
        .nodeTitle(function(n) {
          return nvalue(n).tooltip !== undefined ? nvalue(n).tooltip : diagram.nodeLabel()(n)
        })
        ...
        .edgeLabel(function(e) {
          return e.value.label ? e.value.label.split(/\n|\\n/) : ''
        })
        .edgeStroke(function(e) {
          return e.value.color || 'black'
        })
    ...
    dc_graph.snapshot_graphviz = function(diagram) {
      var xDomain = diagram.x().domain(),
        yDomain = diagram.y().domain()
      return {
        nodes: diagram
          .nodeGroup()
          .all()
    ...
    dc_graph.cola_layout = function(id) {
        ...
        var wnodes = regenerate_objects(
          ...
          function(v1, v) {
            ...
            if (v1.fixed && typeof v.dcg_nodeFixed === 'object') {
              ...
              if (v.x !== undefined) v1.x = v.x
              if (v.y !== undefined) v1.y = v.y
            }
          },
        )
        var wedges = regenerate_objects(
      ...
      function stop() {
        if (_d3cola) _d3cola.stop()
      }

      var graphviz = dc_graph.graphviz_attrs(),
        graphviz_keys = Object.keys(graphviz)
      ...
      var engine = Object.assign(graphviz, {
        ...
        flowLayout: function(flow) {
          if (!arguments.length) {
            ...
            switch (dir) {
              case 'LR':
                return { axis: 'x', minSeparation: engine.ranksep() + engine.parent().nodeRadius() * 2 }
              case 'TB':
                return { axis: 'y', minSeparation: engine.ranksep() + engine.parent().nodeRadius() * 2 }
              default:
    ...
    dc_graph.dagre_layout = function(id) {
      ...
      function init(options) {
        ...
        _dagreGraph.setDefaultEdgeLabel(function() {
          return {}
        })
      }
    ...
    dc_graph.tree_layout = function(id) {
      ...
      function init(options) {
        ...
        _dfs = dc_graph.depth_first_traversal({
          ...
          skip: function(n, indegree) {
            ...
            if (n.hit_ins === indegree) delete n.hit_ins
          },
          finish: function(rows) {
            ...
            rows.forEach(function(row) {
              var sort = row.sort(function(a, b) {
                return a.x - b.x
              })
              var badi = null,
                badl = null,
              ...
              for (var i = 0; i < sort.length - 1; ++i) {
                ...
                if (!badi) {
                  ...
                  want += best_dist(left, right)
                  if (i < sort.length - 2 && right.x < badl + want) continue
                  // still bad
                  else {
                    if (badi > 0) --badi // might want to use more left
                    var l, limit
                    if (i < sort.length - 2) {
                      // found space before right
                      ...
                      l = Math.max(
                        sort[badi].x,
                        badl - best_dist(sort[badi], sort[badi + 1]) - (want - right.x + badl) / 2,
                      )
                      limit = sort.length
                    }
      ...
      var layout = {
        ...
        populateLayoutNode: function(layout, node) {
          if (this.rowFunction()) layout.dcg_rank = this.rowFunction.eval(node)
        },
        populateLayoutEdge: function() {},
        nodeWidth: property(function(n) {
          return n.width
        }),
        offsetX: property(30),
        offsetY: property(30),
    ...
    dc_graph.graphviz_layout = function(id, layout, server) {
      ...
      function data(nodes, edges) {
        ...
        lines = lines.concat(
          nodes.map(function(v) {
            ...
            if (v.dcg_nodeFixed)
              props.push(stringize_property('pos', [v.dcg_nodeFixed.x, 1000 - v.dcg_nodeFixed.y].join(',')))
      ...
      function process_response(error, result) {
        _dispatch.start()
        var bb = result.bb.split(',').map(function(x) {
          return +x
        })
        ...
        _clusters.forEach(function(c) {
          ...
          c.bbflip = c.bb.split(',').map(function(s) {
            return +s
          })
        ...
        var edges = (result.edges || []).map(function(e) {
          ...
          if (e._draw_) {
            var directive = e._draw_.find(function(d) {
              return d.op && d.points
            })
            e2.points = directive.points.map(function(p) {
              return { x: p[0], y: bb[3] - p[1] }
            })
          }
          return e2
        })
    ...
    dc_graph.d3_force_layout = function(id) {
      ...
      function init(options) {
        ...
        _simulation = d3.layout.force().size([options.width, options.height])
        if (options.linkDistance) {
          if (typeof options.linkDistance === 'number') _simulation.linkDistance(options.linkDistance)
      ...
      function installForces() {
        if (_paths === null) _simulation.gravity(_options.gravityStrength).charge(_options.initialCharge)
        else {
      ...
      function runSimulation(iterations) {
        ...
        for (var i = 0; i < 300; ++i) {
          _simulation.tick()
          if (_paths) applyPathAngleForces()
        }
        _simulation.stop()
      }
      ...
      function installForces(paths) {
        if (paths)
          paths = paths.filter(function(path) {
            return path.nodes.every(function(nk) {
              return _nodes[nk]
            })
          })
        if (paths === null || !paths.length) {
          ...
          _simulation.force(
            ...
            d3v4
              ...
              .pathNodes(function(p) {
                return p.nodes
              })
              .pathStrength(function(p) {
                return p.strength
              })
              .paths(paths),
          )
    ...
    dc_graph.flexbox_layout = function(id) {
      ...
      function all_keys(tree) {
        ...
        return Array.prototype.concat.apply(
          [key],
          Object.keys(tree.children).map(function(k) {
      ...
      function create_flextree(attrs, tree) {
        ...
        if (tree.node) Object.assign(attrs, tree.node)
        for (var attr in attrs) {
          if (internal_attrs.includes(attr)) continue
          if (isParent && skip_on_parents.includes(attr)) continue
          var value = attrs[attr]
          if (typeof value === 'function') value = value(tree.node)
        ...
        if (isParent) {
          flexnode.children = Object.values(tree.children)
            .sort(attrs.sort)
            .map(function(c) {
              return c.address[c.address.length - 1]
            })
            .map(function(key) {
      ...
      function apply_layout(offset, tree) {
        if (_engine.logStuff()) console.log(tree.node.dcg_nodeKey + ': ' + JSON.stringify(tree.flexnode.layout))
      ...
      function start() {
        ...
        if (_engine.logStuff()) console.log(JSON.stringify(flexTree, null, 2))
      ...
      var _engine = {
        ...
        populateLayoutNode: function(n1, n) {
          ;['sort', 'order'].concat(supportedAttributes).forEach(function(attr) {
            if (n.orig.value[attr]) n1[attr] = n.orig.value[attr]
          })
        },
        populateLayoutEdge: function() {},
        addressToKey: property(function(ad) {
          return ad.join(',')
        }),
        keyToAddress: property(function(nid) {
          return nid.split(',')
        }),
      ...
      var _engine = {
        ...
        populateLayoutNode: function(n1, n) {
          ;['x', 'y'].forEach(function(attr) {
            if (n.orig.value[attr] !== undefined) n1[attr] = n.orig.value[attr]
          })
        },
        populateLayoutEdge: function() {},
        addressToKey: property(function(ad) {
    ...
    function port_name(nodeId, edgeId, portName) {
      if (!(nodeId || edgeId)) return null // must have one key or the other
    ...
    dc_graph.place_ports = function() {
      function received_layout(diagram, nodes, wnodes, edges, wedges, ports, wports) {
        ...
        function clip(v, bounds) {
          if (is_ccw(v, bounds[0])) return bounds[0]
          else if (is_ccw(bounds[1], v)) return bounds[1]
          else return v
        }
        function a_to_v(a) {
        ...
        for (var nid in node_ports) {
          ...
          nports.forEach(function(p) {
            ...
            if (p.abounds[0] > p.abounds[1]) p.abounds[1] += 2 * Math.PI
          ...
          var inside = [],
            ...
            unplaced = []
          nports.forEach(function(p) {
            if (!p.vec) unplaced.push(p)
            else if (p.vbounds && !in_bounds(p.vec, p.vbounds)) outside.push(p)
            else inside.push(p)
          })
          ...
          for (var b in boundports) {
            var bounds = boundses[b],
              bports = boundports[b]
            if (bports.length === 1) bports[0].vec = a_to_v((bounds[0] + bounds[1]) / 2)
            else {
          ...
          for (var i = 0; i < inside.length; ++i) {
            var x = inside[i]
            if (unplaced.includes(x)) continue
            for (var j = i + 1; j < inside.length; ++j) {
              var y = inside[j]
              if (unplaced.includes(y)) continue
              if (!misses(x, y)) {
                if (!x.edges.length) {
                  unplaced.push(x)
                  continue
                } else unplaced.push(y)
              }
            }
          }
          inside = inside.filter(function(p) {
            return !unplaced.includes(p)
          })

          // place any remaining by trying random spots within the range until it misses all or we give up
          var patience = _mode.patience(),
            maxdist = 0,
            maxvec
          while (unplaced.length) {
            ...
            project_port(diagram, n, p)
            var mindist = d3.min(inside, function(p2) {
      ...
      var _mode = {
        parent: property(null).react(function(p) {
          ...
          } else if (_mode.parent()) _mode.parent().on('receivedLayout.place-ports', null)
        }),
    ...
    dc_graph.grid = function() {
      ...
      function remove(diagram, node, edge, ehover) {
        if (_gridLayer) _gridLayer.remove()
      }

      function draw(diagram) {
        _gridLayer = diagram
          .g()
          .selectAll('g.grid-layer')
          .data([0])
        _gridLayer
          .enter()
          .append('g')
        ...
        var vline_data =
          _scale >= _mode.threshold() ? d3.range(Math.floor(_xDomain[0]), Math.ceil(_xDomain[1]) + 1) : []
        var vlines = _gridLayer.selectAll('line.grid-line.vertical').data(vline_data, function(d) {
          return d - ofs
        })
        vlines.exit().remove()
        vlines
        ...
        var hline_data =
          _scale >= _mode.threshold() ? d3.range(Math.floor(_yDomain[0]), Math.ceil(_yDomain[1]) + 1) : []
        var hlines = _gridLayer.selectAll('line.grid-line.horizontal').data(hline_data, function(d) {
          return d - ofs
        })
        hlines.exit().remove()
        hlines
    ...
    dc_graph.troubleshoot = function() {
      ...
      function draw(diagram, node, edge, ehover) {
        ...
        var textbounds = node
          .data()
          .map(function(n) {
            if (!n.bbox || (!n.bbox.width && !n.bbox.height)) return null
        ...
        var radiibounds = node
          .data()
          .map(function(n) {
            if (typeof n.dcg_rx !== 'number') return null
        ...
        var heads = _mode.arrowLength()
          ? edge.data().map(function(e) {
        ...
        var headpts = Array.prototype.concat.apply(
          [],
          edge.data().map(function(e) {
            var arrowSize = diagram.edgeArrowSize.eval(e)
        ...
        var tailpts = Array.prototype.concat.apply(
          [],
          edge.data().map(function(e) {
            var arrowSize = diagram.edgeArrowSize.eval(e)
            return edge_arrow_points(
      ...
      function draw_arrow_orient(binding, classname, color, markerUrl) {
        ...
        binding.attr({
          ...
          x2: function(d) {
            return d.pos.x - Math.cos(unrad(d.orient)) * _mode.arrowLength()
          },
          y2: function(d) {
            return d.pos.y - Math.sin(unrad(d.orient)) * _mode.arrowLength()
          },
          stroke: color,
      ...
      function edge_arrow_points(arrows, defn, arrowSize, stemWidth, orient, endp, strokeWidth) {
        ...
        endp = [endp.x, endp.y]
        if (!parts.length) return [[endp[0] - (xunit[0] * strokeWidth) / 2, endp[1] - (xunit[1] * strokeWidth) / 2]]
        var globofs = add_points([-strokeWidth / arrowSize / 2, 0], mult_point(front_ref(parts[0].frontRef), -1))
        var pts = offsets.map(function(ofs, i) {
          return mult_point([globofs, front_ref(parts[i].frontRef), ofs.offset].reduce(add_points), arrowSize)
        })
        pts.push(
          mult_point(
            [globofs, back_ref(parts[parts.length - 1].backRef), offsets[parts.length - 1].offset].reduce(add_points),
            arrowSize,
          ),
        )
        return pts.map(function(p) {
          return add_points(endp, [p[0] * xunit[0] - p[1] * xunit[1], p[0] * xunit[1] + p[1] * xunit[0]])
        })
      }

      function draw_x(binding, classname, color) {
        var xw = _mode.xWidth() / 2,
          xh = _mode.xHeight() / 2
        binding.exit().remove()
        binding
          .enter()
      ...
      function remove(diagram, node, edge, ehover) {
        if (_debugLayer) _debugLayer.remove()
      }
      ...
      var _mode = dc_graph.mode('highlight-paths', {
        ...
        parent: function(p) {
          ...
          } else if (_mode.parent()) _mode.parent().on('zoomed.troubleshoot', null)
        },
      })
    ...
    dc_graph.validate = function(title) {
      function falsy(objects, accessor, what, who) {
        ...
        return f.length ? [what + ' is empty for ' + f.length + ' of ' + objects.length + ' ' + who, f] : null
      }
      ...
      function validate() {
        ...
        } else console.log('validation of ' + title + ' succeeded with ' + count_text() + '.')
      }
      var _mode = {
        parent: property(null).react(function(p) {
          if (p) p.on('data.validate', validate)
          else _mode.parent().on('data.validate', null)
        }),
      }
...
**/
    dc_graph.legend = function(legend_namespace) {
      ...
      var _legend = dc_graph.mode(legend_namespace, {
        ...
        parent: function(p) {
          if (p) {
            p.on('render.' + legend_namespace, render).on('data.' + legend_namespace, on_data)
          } else {
            _legend
              .parent()
    ...
    **/
      ...
      _legend.replaceFilter = function(filter) {
        if (filter && filter.length === 1) _included = filter[0]
        else _included = []
        return _legend
      }
     ...
     **/
      ...
      function redraw() {
        ...
        var items =
          !_legend.omitEmpty() || !_counts
            ? _items
            : _items.filter(function(i) {
                return (_included.length && !_included.includes(i.orig.key)) || _counts[i.orig.key]
              })
        var item = legend.selectAll(_legend.type().itemSelector()).data(items, function(n) {
          return n.name
        })
        item.exit().remove()
        ...
        item.attr('transform', function(n, i) {
        ...
        item
          ...
          .text(function(d) {
            return (
              d.name +
              (_legend.counter() && _counts
                ? ' (' +
                  (_counts[d.orig.key] || 0) +
                  (_counts[d.orig.key] !== _totals[d.orig.key] ? '/' + (_totals[d.orig.key] || 0) : '') +
                  ')'
                : '')
            )
          })
        _legend.type().draw(_legend.parent(), itemEnter, item)
        if (_legend.noLabel()) item.selectAll(_legend.type().labelSelector()).remove()

        if (_legend.dropdown()) {
          var caret = item.selectAll('text.dropdown-caret').data(function(x) {
            return [x]
          })
          caret
            .enter()
        ...
        if (_legend.dimension()) {
          item.attr('cursor', 'pointer').on('click.' + legend_namespace, function(d) {
            var key = _legend.parent().nodeKey.eval(d)
            if (!_included.length) _included = _items.map(_legend.parent().nodeKey.eval)
            if (_included.includes(key))
              _included = _included.filter(function(x) {
                return x !== key
              })
            else _included.push(key)
            apply_filter()
            _dispatch.filtered(_legend, key)
          })
        } else {
          item.attr('cursor', 'auto').on('click.' + legend_namespace, null)
        }
        item
          .transition()
          .duration(1000)
      ...
      function render() {
        ...
        if (exemplars instanceof Array) {
          _items = exemplars.map(function(v) {
            return { name: v.name, orig: { key: v.key, value: v.value }, cola: {} }
          })
        } else {
          _items = []
          for (var item in exemplars) _items.push({ name: item, orig: { key: item, value: exemplars[item] }, cola: {} })
        }
        redraw()
      }
      ...
      /* enables filtering */
      _legend.dimension = property(null).react(function(v) {
        if (!v) {
          _included = []
          apply_filter()
        }
    ...
    dc_graph.legend.node_legend = function() {
      return {
        ...
        create: function(diagram, selection) {
          return selection.append('g').attr('class', 'node')
        },
        draw: function(diagram, itemEnter, item) {
          diagram._enterNode(itemEnter)._updateNode(item)
        },
      }
    }
    ...
    dc_graph.legend.edge_legend = function() {
      var _type = {
        ...
        create: function(diagram, selection, w, h) {
          var edgeEnter = selection
            .append('g')
            .attr('class', 'edge-container')
            .attr('opacity', 0)
          edgeEnter.append('rect').attr({
            x: -w / 2,
            y: -h / 2,
            width: w,
            height: h,
          ...
          edgeEnter.append('svg:path').attr({
            class: 'edge',
            id: function(d) {
              return d.name
            },
    ...
    dc_graph.constraint_pattern = function(pattern) {
      ...
      pattern.nodes.forEach(function(n) {
        ...
        } else if (n.match) type.match = n.match
      ...
      return function(diagram, nodes, edges) {
        ...
        edges.forEach(function(e) {
          ...
          edge_rules.forEach(function(r) {
            if (
              members[r.source] &&
              members[r.source].whether[source] &&
              members[r.target] &&
              members[r.target].whether[target]
            ) {
          ...
          rows.forEach(function(row) {
            ...
            for (var i = 0; i < sort.length - 1; ++i) {
              ...
              if (!badi) {
                ...
                want += best_dist(left, right)
                if (i < sort.length - 2 && right.cola.x < badl + want) continue
                // still bad
                else {
                  if (badi > 0) --badi // might want to use more left
                  var l, limit
                  if (i < sort.length - 2) {
                    ...
                    l = Math.max(
                      sort[badi].cola.x,
                      badl - best_dist(sort[badi], sort[badi + 1]) - (want - right.cola.x + badl) / 2,
                    )
                    limit = sort.length
                  }
                  for (var j = badi + 1; j < limit; ++j) {
     ...
     **/
      _mode.parent = property(null).react(function(p) {
        var diagram
        if (p) {
          var first = true
          diagram = p
          p.on(_eventName + '.' + event_namespace, function(node, edge, ehover) {
            ...
            if (first && options.first) {
              ...
              first = false
            } else if (options.rest) options.rest(diagram, node, edge, ehover)
          })
    ...
    dc_graph.tip = function(options) {
      ...
      function init(parent) {
        if (!_d3tip) {
          ...
          if (_mode.offset()) _d3tip.offset(_mode.offset())
            ...
            _mode.content()(d, function(content) {
              ...
              d3.select('div.d3-tip')
                ...
                .on('click.' + _namespace, function() {
                  ...
                  if (_mode.linkCallback()) _mode.linkCallback()(this.id)
                })
              _dispatch.tipped(d)
            })
          }
        if (_hideTimeout) window.clearTimeout(_hideTimeout)
        if (_mode.delay()) {
      ...
      function hide_tip() {
        if (!check_hide_tip.apply(this)) return
        preempt_tip()
        _d3tip.hide()
      }
      ...
      _mode.displayTip = function(filter, n, cb) {
        ...
        var found = _mode
          .selection()
          .select(_mode.parent(), _mode.parent().selectAllNodes(), _mode.parent().selectAllEdges(), null)
          .filter(filter)
        if (found.size() > 0) {
          ...
          if (cb) cb(d)
          if (_mode.programmatic()) found.on('mouseout.' + _namespace, hide_tip_delay)
        }
        return _mode
      }
    ...
    dc_graph.tip.table = function() {
      var gen = function(d, k) {
        d = gen.fetch()(d)
        if (!d) return // don't display tooltip if no content
        var data, keys
        if (Array.isArray(d)) data = d
        else if (typeof d === 'number' || typeof d === 'string') data = [d]
        else {
          // object
          data = keys = Object.keys(d)
            .filter(d3.functor(gen.filter()))
            .filter(function(k) {
      ...
      var _dropdown = {
        ...
        show: function(key, x, y) {
          var dropdown = _dropdown
            .parent()
            .root()
            .selectAll('div.dropdown.' + _dropdown.id)
            .data([0])
          ...
          if (_dropdown.scrollHeight()) {
            ...
            dropdown.style('max-height', height).property('scrollTop', 0)
            dropdownEnter
          ...
          var values = _dropdown.fetchValues()(key, function(values) {
            var items = container.selectAll('div.dropdown-item').data(values)
            items
              .enter()
              .append('div')
            ...
            if (selects.includes('click')) select_event = 'click'
            else if (selects.includes('hover')) select_event = 'mouseenter'
            items.text(function(item) {
              return _dropdown.itemText()(item)
            })
            if (select_event) {
              items.on(select_event + '.select', function(d) {
                _dropdown.itemSelected()(d)
              })
            }
            if (hides.includes('clickitem')) {
              items.on('click.hide', function(d) {
                capture.remove()
                dropdown.style('visibility', 'hidden')
              })
            }
        ...
        height: property(10),
        itemText: property(function(x) {
          return x
        }),
        itemSelected: property(function() {}),
        fetchValues: property(function(key, k) {
          k([])
        }),
    ...
    dc_graph.keyboard = function() {
      var _input_anchor,
        _dispatch = d3.dispatch('keydown', 'keyup')
      ...
      function draw(diagram) {
        _input_anchor = diagram
          .svg()
          .selectAll('a#dcgraph-keyboard')
          .data([1])
        _input_anchor
          .enter()
    ...
    dc_graph.brush = function() {
      var _brush = null,
        _gBrush,
        _dispatch = d3.dispatch('brushstart', 'brushmove', 'brushend')
    ...
    dc_graph.select_things = function(things_group, things_name, thinginess) {
      ...
      function selection_changed(diagram) {
        return function(selection, refresh) {
          ...
          _selected = selection
          if (refresh) diagram.requestRefresh()
        }
      }
      var _have_bce = false
      function background_click_event(diagram, v) {
        ...
        if (_have_bce === v) return
        diagram.svg().on(
          'click.' + things_name,
          v
            ? function(t) {
                if (d3.event.target === this) things_group.set_changed([])
              }
            : null,
        )
        _have_bce = v
      ...
      function brushstart() {
        if (isUnion(d3.event.sourceEvent) || isToggle(d3.event.sourceEvent)) _oldSelected = _selected.slice()
        else {
          _oldSelected = []
      ...
      function brushmove(ext) {
        if (!thinginess.intersectRect) return
        var rectSelect = thinginess.intersectRect(ext)
        var newSelected
        if (isUnion(d3.event.sourceEvent)) newSelected = rectSelect.reduce(add_array, _oldSelected)
        else if (isToggle(d3.event.sourceEvent)) newSelected = rectSelect.reduce(toggle_array, _oldSelected)
        else newSelected = rectSelect
        things_group.set_changed(newSelected)
      }

      function draw(diagram, node, edge) {
        var condition = _mode.noneIsAll()
          ? function(t) {
        ...
        thinginess.clickables(diagram, node, edge).on('mouseup.' + things_name, function(t) {
          if (thinginess.excludeClick && thinginess.excludeClick(d3.event.target)) return
          ...
          if (_mousedownThing !== t) return
          var key = thinginess.key(t),
            newSelected
          if (_mode.multipleSelect()) {
            if (isUnion(d3.event)) newSelected = add_array(_selected, key)
            else if (isToggle(d3.event)) newSelected = toggle_array(_selected, key)
          }
          if (!newSelected) newSelected = [key]
          things_group.set_changed(newSelected)
        })
        ...
        if (_mode.multipleSelect()) {
          ...
          brush_mode.activate()
        } else background_click_event(diagram, _mode.clickBackgroundClears())

        if (_mode.autoCropSelection()) {
          // drop any selected which no longer exist in the diagram
          var present = thinginess
            .clickables(diagram, node, edge)
            .data()
            .map(thinginess.key)
          var now_selected = _selected.filter(function(k) {
            return contains(present, k)
          })
          if (_selected.length !== now_selected.length) things_group.set_changed(now_selected, false)
        }
      }
      ...
      var _mode = dc_graph.mode(things_name, {
        ...
        parent: function(p) {
          ...
          if (p && _mode.multipleSelect()) {
            ...
            brush_mode.on('brushstart.' + things_name, brushstart).on('brushmove.' + things_name, brushmove)
          }
        },
      ...
      _mode.clickBackgroundClears = property(true, false).react(function(v) {
        if (!_mode.multipleSelect() && _mode.parent()) background_click_event(_mode.parent(), v)
      })
    ...
    dc_graph.select_nodes = function(props, options) {
      options = options || {}
      var select_nodes_group = dc_graph.select_things_group(
      ...
      var thinginess = {
        intersectRect: function(ext) {
          return _mode
            ...
            .data()
            .filter(function(n) {
              return n && ext[0][0] < n.cola.x && n.cola.x < ext[1][0] && ext[0][1] < n.cola.y && n.cola.y < ext[1][1]
            })
            .map(this.key)
        },
    ...
    dc_graph.select_edges = function(props, options) {
      options = options || {}
      var select_edges_group = dc_graph.select_things_group(
          ...
          return this.clickables()
            ...
            .filter(function(e) {
              ...
              return [sp, tp].some(function(p) {
                return ext[0][0] < p.x && p.x < ext[1][0] && ext[0][1] < p.y && p.y < ext[1][1]
              })
            })
            .map(this.key)
        },
    ...
    dc_graph.select_ports = function(props, options) {
      ...
      var select_ports_group = dc_graph.select_things_group(
      ...
      var thinginess = {
        ...
        applyStyles: function(pred) {
          _mode
            ...
            .cascade(50, true, conditional_properties(pred, props))
        },
        removeStyles: function() {
          _mode
            .parent()
    ...
    dc_graph.move_nodes = function(options) {
      ...
      function draw(diagram, node, edge) {
        node.on('mousedown.move-nodes', function(n) {
          ...
          if (_drawGraphs && _drawGraphs.usePorts() && _drawGraphs.usePorts().eventPort()) return
        ...
        function mouse_move() {
          if (_startPos) {
            ...
            if (_maybeSelect) select_nodes_group.set_changed([_maybeSelect])
            ...
            if (!_moveStarted && Math.hypot(dx, dy) > _mode.dragSize()) {
              ...
              if (_downNode) _downNode.style('pointer-events', 'none')
            }
            if (_moveStarted) {
              for_each_selected(function(n) {
        ...
        function mouse_up() {
          if (_startPos) {
            ...
            if (_brush) _brush.activate()
            _startPos = null
          }
        }
        node.on('mousemove.move-nodes', mouse_move).on('mouseup.move-nodes', mouse_up)
        diagram
          .svg()
    ...
    dc_graph.fix_nodes = function(options) {
      ...
      function find_changes() {
        ...
        _wnodes.forEach(function(n) {
          var key = _mode.parent().nodeKey.eval(n),
            ...
            changed = false
          if (oldFixed) {
            if (!fixPos || fixPos.x !== oldFixed.x || fixPos.y !== oldFixed.y) changed = true
          } else changed = fixPos
          if (changed) changes.push({ n: n, fixed: fixPos ? { x: fixPos.x, y: fixPos.y } : null })
        })
        return changes
      }
      function execute_change(n, fixed) {
        if (fixed) _execute.fix_node(n.orig.value, fixed)
        else _execute.unfix_node(n.orig.value)
      }
      function tell_then_set(changes) {
        ...
        var promises = changes.map(function(change) {
          ...
          return callback(key, change.fixed).then(function(fixed) {
      ...
      function on_data(diagram, nodes, wnodes, edges, wedges, ports, wports) {
        ...
        if (_mode.strategy().on_data) {
          ...
          if (_mode.reportOverridesAsynchronously()) tell_changes(changes) // dangling promise
        }
      }
      ...
      var _mode = {
        parent: property(null).react(function(p) {
          ...
          } else if (_mode.parent()) _mode.parent().on('data.fix-nodes', null)
        }),
        ...
        on_data: function(exec, nodes, wnodes, edges, wedges, ports, wports) {
          ...
          wnodes.forEach(function(n) {
            var nid = exec.nodeid(n),
              pos = exec.get_fix(n)
            if (pos && !_allFixes[nid]) _allFixes[nid] = { id: nid, age: _age, pos: pos }
          })
          ...
          components.forEach(function(comp, i) {
            var oldcomps = comp.reduce(function(cc, n) {
              if (n.last_component) {
                ...
                counts.total++
                if (_allFixes[exec.nodeid(n)]) counts.fixed++
              }
              return cc
            }, {})
            var fixed_by_size = Object.keys(oldcomps)
              .reduce(function(ff, compid) {
            ...
            comp.forEach(function(n) {
              var nid = exec.nodeid(n)
              if (!kept[nid]) _allFixes[nid] = null
      ...
      function selection_changed(diagram) {
        return function(selection) {
          if (selection.length) {
            ...
            _mode
              .dimensionAccessor()(diagram)
              .filterFunction(function(k) {
                return set.has(k)
              })
          } else
            _mode
    ...
    dc_graph.delete_things = function(things_group, mode_name, id_tag) {
      ...
      function delete_selection(selection) {
        if (!_mode.crossfilterAccessor()) throw new Error('need crossfilterAccessor')
        if (!_mode.dimensionAccessor()) throw new Error('need dimensionAccessor')
        ...
        if (_mode.onDelete()) promise = promise.then(_mode.onDelete())
      ...
      function draw(diagram) {
        _keyboard.on('keyup.' + mode_name, function() {
          if (d3.event.code === _deleteKey) delete_selection()
        })
      }
      function remove(diagram) {}
      var _mode = dc_graph.mode(mode_name, {
        ...
        parent: function(p) {
          ...
          if (p) {
            ...
            if (!_keyboard) p.child('keyboard', (_keyboard = dc_graph.keyboard()))
          }
        },
      })
    ...
    dc_graph.delete_nodes = function(id_tag, options) {
      ...
      _mode.preDelete(function(nodes) {
        ...
        if (!deleteEdgesMode) return null // reject if we can't delete the edges
        ...
        return Promise.resolve(undefined).then(function() {
          var deleteEdges = diagram
            .edgeGroup()
            .all()
            .filter(function(e) {
              return nodes.indexOf(diagram.edgeSource()(e)) !== -1 || nodes.indexOf(diagram.edgeTarget()(e)) !== -1
            })
            .map(diagram.edgeKey())
      ...
      function draw(diagram, node, edge) {
        _keyboard.on('keyup.' + options.label_type, function() {
          if (_selected.length) {
            ...
            if (d3.event.key.length !== 1) return
        ...
        if (_selectThings)
          _selectThings
            .thinginess()
            .clickables(diagram, node, edge)
            .on('dblclick.' + options.label_type, function() {
    ...
    dc_graph.highlight_neighbors = function(includeprops, excludeprops, neighborsgroup, thingsgroup) {
      var highlight_neighbors_group = dc_graph.register_highlight_neighbors_group(
      ...
      function remove(diagram, node, edge) {
        node.on('mouseover.highlight-neighbors', null).on('mouseout.highlight-neighbors', null)
        highlight_neighbors_group.highlight_node(null)
      }
      ...
      var _mode = dc_graph.mode('highlight-neighbors', {
        ...
        parent: function(p) {
          ...
          if (p && !p.child('highlight-things'))
            p.child(
              'highlight-things',
              dc_graph.highlight_things(includeprops, excludeprops).durationOverride(_mode.durationOverride()),
            )
        },
      })
      _mode.durationOverride = property(undefined)
    ...
    dc_graph.highlight_radius = function(options) {
      ...
      var highlight_things_group = dc_graph.register_highlight_things_group(
      ...
      function selection_changed(nodes) {
        ...
        if (!Object.keys(nodeset).length && !Object.keys(edgeset).length) nodeset = edgeset = null
        highlight_things_group.highlight(nodeset, edgeset)
      }
      ...
      var _mode = {
        parent: function(p) {
          ...
          } else if (_mode.parent()) _mode.parent().on('data.highlight-radius', null)
    ...
    dc_graph.highlight_paths = function(pathprops, hoverprops, selectprops, pathsgroup) {
      ...
      function paths_changed(nop, eop) {
        ...
        if (
          Object.keys(node_on_paths).length === 0 &&
          Object.keys(nop).length === 0 &&
          Object.keys(edge_on_paths).length === 0 &&
          Object.keys(eop).length === 0
        )
          return
        node_on_paths = nop
        edge_on_paths = eop
      ...
      function intersect_paths(pathsA, pathsB) {
        if (!pathsA || !pathsB) return false
        return pathsA.some(contains_path(pathsB))
      }

      function toggle_paths(pathsA, pathsB) {
        if (!pathsA) return pathsB
        else if (!pathsB) return pathsA
        if (pathsB.every(contains_path(pathsA))) return pathsA.filter(doesnt_contain_path(pathsB))
    ...
    dc_graph.spline_paths = function(pathreader, pathprops, hoverprops, selectprops, pathsgroup) {
      ...
      function paths_changed(nop, eop, paths) {
        ...
        if (localPaths.length) {
          var nidpaths = localPaths.map(function(lpath) {
            ...
            if (_selected && _selected.indexOf(lpath) !== -1) strength *= _mode.selectedStrength()
            return {
          ...
          engine.paths(null)
          if (_savedPositions) engine.restorePositions(_savedPositions)
        }
        if (_selected)
          _selected = _selected.filter(function(p) {
            return localPaths.indexOf(p) !== -1
          })
        _mode.parent().redraw()
      }
      ...
      function path_keys(path, unique) {
        unique = unique !== false
        var keys = pathreader.elementList
          .eval(path)
      ...
      function insertDummyNodes(path_coord) {
        ...
        for (var i = 0; i < path_coord.length; i++) {
          if (i - 1 >= 0 && i + 1 < path_coord.length) {
            if (path_coord[i - 1].x === path_coord[i + 1].x && path_coord[i - 1].y === path_coord[i + 1].y) {
              // insert node when the previous and next nodes are the same
              var x1 = path_coord[i - 1].x,
                y1 = path_coord[i - 1].y
              var x2 = path_coord[i].x,
                y2 = path_coord[i].y
              var dx = x1 - x2,
                dy = y1 - y2
      ...
      // helper functions
      var vecDot = function(v0, v1) {
        return v0.x * v1.x + v0.y * v1.y
      }
      var vecMag = function(v) {
        return Math.sqrt(v.x * v.x + v.y * v.y)
      }
      ...
      function drawSpline(paths) {
        ...
        // edge spline
        var edge = _layer.selectAll('.spline-edge').data(paths, function(path) {
          return path_keys(path).join(',')
        })
        edge.exit().remove()
        var edgeEnter = edge
          .enter()
          ...
          .attr('id', function(d, i) {
            return 'spline-path-' + i
          })
          .attr('stroke-width', pathprops.edgeStrokeWidth || 1)
          .attr('fill', 'none')
          .attr('d', function(d) {
            return genPath(d, true, pathprops.lineTension, _mode.avoidSharpTurns())
          })
        edge
          ...
          .attr('opacity', function(p) {
            return (
              ...
              pathprops.edgeOpacity ||
              1
            )
          })
        function path_order(p) {
          return hoverpaths.indexOf(p) !== -1 ? 2 : selected.indexOf(p) !== -1 ? 1 : 0
        }
        edge.sort(function(a, b) {
          return path_order(a) - path_order(b)
        })
        _layer.selectAll('.spline-edge-hover').each(function() {
          this.parentNode.appendChild(this)
        })
        edge
          .transition()
          .duration(_mode.parent().transitionDuration())
          .attr('d', function(d) {
            return genPath(d, false, pathprops.lineTension, _mode.avoidSharpTurns())
          })

        // another wider copy of the edge just for hover events
        var edgeHover = _layer.selectAll('.spline-edge-hover').data(paths, function(path) {
          return path_keys(path).join(',')
        })
        edgeHover.exit().remove()
        var edgeHoverEnter = edgeHover
          ...
          .on('click.spline-paths', function(d) {
            ...
            else if (d3.event.shiftKey) selected.push(d)
            else selected = [d]
    ...
    dc_graph.draw_spline_paths = deprecate_function(
      'draw_spline_paths has been renamed spline_paths, please update',
      dc_graph.spline_paths,
    )
    ...
    dc_graph.expand_collapse = function(options) {
      ...
      var _keyboard,
        ...
        _overEdge,
        _expanded = {}
      var expanded_highlight_group = dc_graph.register_highlight_things_group(
        options.expanded_highlight_group || 'expanded-highlight-group',
      )
      var collapse_highlight_group = dc_graph.register_highlight_things_group(
        options.collapse_highlight_group || 'collapse-highlight-group',
      )
      var hide_highlight_group = dc_graph.register_highlight_things_group(
      ...
      if (options.dirs.length > 2) throw new Error('there are only two directions to expand in')

      var _gradients_added = {}
      function add_gradient_def(color, diagram) {
        if (_gradients_added[color]) return
      ...
      function spike_directioner(rankdir, dir, N) {
        if (dir === 'both')
          ...
          if (dir === 'in') ofs += Math.PI
          return function(i) {
            return ofs + sweep * (-0.5 + (N > 1 ? i / (N - 1) : 0)) // avoid 0/0
          }
        }
      }

      function draw_stubs(diagram, node, edge, n, spikes) {
        if (n && _expanded[spikes.dir][diagram.nodeKey.eval(n)]) spikes = null
        var spike = node.selectAll('g.spikes').data(function(n2) {
          return spikes && n === n2 ? [n2] : []
        })
        spike.exit().remove()
        spike
          .enter()
          .insert('g', ':first-child')
          .classed('spikes', true)
        var rect = spike.selectAll('rect.spike').data(function(n) {
          var key = diagram.nodeKey.eval(n)
          var dir = spikes.dir,
            N = spikes.n,
          ...
          for (var i = 0; i < N; ++i) {
            ...
            ret[i] = {
              a: (a * 180) / Math.PI,
              x: Math.cos(a) * n.dcg_rx * 0.9,
              y: Math.sin(a) * n.dcg_ry * 0.9,
      ...
      function highlight_collapse(diagram, n, node, edge, dir) {
        var nk = diagram.nodeKey.eval(n)
        var p
        if (options.get_edges) p = Promise.resolve(options.get_edges(nk, dir))
        else p = Promise.resolve(options.get_degree(nk, dir))
        p.then(function(de) {
          var degree, edges
          if (typeof de === 'number') degree = de
          else {
            edges = de
            degree = edges.length
          }
          ...
          if (edges) {
            ...
            spikes.invisible = edges.filter(function(e) {
              return !shown[diagram.edgeKey()(e)]
            })
          }
          draw_stubs(diagram, node, edge, n, spikes)
          var collapse_nodes_set = {},
            collapse_edges_set = {}
      ...
      function draw(diagram, node, edge, ehover) {
        function over_node(n) {
          ...
          _overNode = n
          _overDir = dir
          if (options.hideNode && detect_key(options.hideKey)) highlight_hiding_node(diagram, n, edge)
          ...
          } else highlight_collapse(diagram, n, node, edge, dir)
        }
        ...
        function click_node(n) {
          var nk = diagram.nodeKey.eval(n)
          if (options.hideNode && detect_key(options.hideKey)) options.hideNode(nk)
          else if (detect_key(options.linkKey)) {
            if (_mode.nodeURL.eval(n) && _mode.urlOpener) _mode.urlOpener()(_mode, n, _mode.nodeURL.eval(n))
          } else {
            clear_stubs(diagram, node, edge)
            var dir = zonedir(diagram, d3.event, options.dirs, n)
        ...
        function enter_edge(e) {
          _overEdge = e
          if (options.hideEdge && detect_key(options.hideKey)) highlight_hiding_edge(diagram, e)
        }
        ...
        function click_edge(e) {
          if (options.hideEdge && detect_key(options.hideKey)) options.hideEdge(diagram.edgeKey.eval(e))
        }
        ...
        _keyboard
          .on('keydown.expand-collapse', function() {
            if (
              d3.event.key === options.hideKey &&
              ((_overNode && options.hideNode) || (_overEdge && options.hideEdge))
            ) {
              if (_overNode) highlight_hiding_node(diagram, _overNode, edge)
              if (_overEdge) highlight_hiding_edge(diagram, _overEdge)
              clear_stubs(diagram, node, edge)
              collapse_highlight_group.highlight({}, {})
      ...
      function expand(dir, nk, whether) {
        ...
        else _expanded[dir][nk] = whether
        var bothmap
        if (_expanded.both) bothmap = _expanded.both
        else {
          bothmap = Object.keys(_expanded.in)
            .filter(function(nk2) {
        ...
        if (dir === 'both' && !_expanded.both)
          options.dirs.forEach(function(dir2, i) {
            if (whether) options.expand(nk, dir2, i !== options.dirs.length - 1)
            else options.collapse(nk, dir2, i !== options.dirs.length - 1)
          })
        else {
          if (whether) options.expand(nk, dir)
          else options.collapse(nk, dir)
        }
      }
      ...
      _mode.clickableLinks = deprecated_property(
    ...
    dc_graph.expand_collapse.shown_hidden = function(opts) {
      var options = Object.assign(
        {
          nodeKey: function(n) {
            return n.key
          }, // this one is raw rows, others are post-crossfilter-group
          edgeKey: function(e) {
            return e.key
          },
      ...
      function is_collapsible(n1, n2) {
        return options.edgeGroup.all().every(function(e2) {
          var n3
          if (options.edgeSource(e2) === n2) n3 = options.edgeTarget(e2)
          else if (options.edgeTarget(e2) === n2) n3 = options.edgeSource(e2)
      ...
      if (options.directional)
        Object.assign(_strategy, {
          get_degree: function(nk, dir) {
            switch (dir) {
              case 'out':
                return out_edges(nk).length
              case 'in':
                return in_edges(nk).length
              default:
            ...
            switch (dir) {
              case 'out':
                out_edges(nk).forEach(function(e) {
                  if (!_nodeHidden[options.edgeTarget(e)]) _nodeShown[options.edgeTarget(e)] = true
                })
                break
              case 'in':
                in_edges(nk).forEach(function(e) {
                  if (!_nodeHidden[options.edgeSource(e)]) _nodeShown[options.edgeSource(e)] = true
                })
                break
              default:
                throw new Error('unknown direction ' + dir)
          ...
          expandedNodes: function(_) {
            if (!arguments.length) throw new Error('not supported') // should not be called
            var that = this
            _nodeShown = {}
        ...
        Object.assign(_strategy, {
          ...
          expand: function(nk) {
            ...
            adjacent_nodes(nk).forEach(function(nk) {
              if (!_nodeHidden[nk]) _nodeShown[nk] = true
            })
            apply_filter()
            dc.redrawAll()
          },
    ...
    dc_graph.expand_collapse.expanded_hidden = function(opts) {
      ...
      function apply_filter() {
        ...
        _edgeDim &&
          _edgeDim.filterFunction(function(ek) {
      ...
      var _strategy = {
        ...
        expandedNodes: function(_) {
          if (!arguments.length) return _nodeExpanded
          _nodeExpanded = _
          apply_filter()
          dc.redrawAll()
          return this
        ...
        collapsibles: function(nk, dir) {
          ...
          delete whatif[nk]
          var shown = get_shown(_nodeExpanded),
            would = get_shown(whatif)
        ...
        hideEdge: function(ek) {
          if (!options.edgeCrossfilter) console.warn('expanded_hidden needs edgeCrossfilter to hide edges')
          _edgeHidden[ek] = true
          apply_filter()
          dc.redrawAll()
        },
    ...
    dc_graph.draw_graphs = function(options) {
      var select_nodes_group = dc_graph.select_things_group(
        ...
        select_edges_group = dc_graph.select_things_group(
          options.select_edges_group || 'select-edges-group',
          'select-edges',
        ),
      ...
      function update_crossout() {
        var data
        if (_crossout) {
          if (_mode.usePorts()) data = [port_pos(_crossout)]
          else data = [{ x: _crossout.node.cola.x, y: _crossout.node.cola.y }]
        } else data = []

        var size = _mode.crossSize(),
          wid = _mode.crossWidth()
      ...
      function create_node(diagram, pos, data) {
        if (!_mode.nodeCrossfilter()) throw new Error('need nodeCrossfilter')
        var node,
          callback = _mode.addNode() || promise_identity
        if (data) node = data
        else {
          node = {}
        ...
        if (pos) fix_nodes_group.new_node(node[_nodeIdTag], node, { x: pos[0], y: pos[1] })
      ...
      function create_edge(diagram, source, target) {
        if (!_mode.edgeCrossfilter()) throw new Error('need edgeCrossfilter')
        var edge = {},
          callback = _mode.addEdge() || promise_identity
      ...
      function check_invalid_drag(coords) {
        ...
        if (!(d3.event.buttons & 1)) {
          ...
          _crossout = null
          if (_mode.conduct().cancelDragEdge) _mode.conduct().cancelDragEdge(_sourceDown)
          erase_hint()
          update_crossout()
          return true
        }
        if (
          !_sourceDown.started &&
          Math.hypot(coords[0] - _hintData[0].source.x, coords[1] - _hintData[0].source.y) > _mode.dragSize()
        ) {
      ...
      function draw(diagram, node, edge, ehover) {
        ...
        if (select_nodes) {
          if (_mode.clickCreatesNodes()) select_nodes.clickBackgroundClears(false)
        }
        node
          .on('mousedown.draw-graphs', function(n) {
            d3.event.stopPropagation()
            if (!_mode.dragCreatesEdges()) return
            if (options.tipsDisable)
              options.tipsDisable.forEach(function(tip) {
                tip.hideTip().disabled(true)
              })
            if (_mode.usePorts()) {
              var activePort
          ...
          .on('mousemove.draw-graphs', function(n) {
            ...
            if (_sourceDown) {
              ...
              if (check_invalid_drag(coords)) return
              ...
              if (n === _sourceDown.node) {
                ...
                if (typeof _mode.usePorts() === 'object' && _mode.usePorts().eventPort)
                  ...
                  activePort =
                    diagram.getPort(diagram.nodeKey.eval(n), null, 'in') ||
                    diagram.getPort(diagram.nodeKey.eval(n), null, 'out')
                if (activePort) _targetMove = { node: n, port: activePort }
                else _targetMove = null
              ...
              if (_mode.conduct().changeDragTarget) {
                ...
                if (change)
                  if (_mode.conduct().changeDragTarget(_sourceDown, _targetMove)) {
                    _crossout = null
                    if (options.negativeTip) options.negativeTip.hideTip()
                    msg = (_mode.conduct().validTargetMessage && _mode.conduct().validTargetMessage()) || 'matches'
                    ...
                    _crossout = _mode.usePorts() ? _targetMove && _targetMove.port : _targetMove && _targetMove.node
                    if (_targetMove && _mode.conduct().invalidTargetMessage) {
                      if (options.positiveTip) options.positiveTip.hideTip()
              ...
              if (_targetMove) {
                if (_targetMove.port) _hintData[0].target = port_pos(activePort)
                else _hintData[0].target = { x: n.cola.x, y: n.cola.y }
              } else {
          ...
          .on('mouseup.draw-graphs', function(n) {
            _crossout = null
            if (options.negativeTip) options.negativeTip.hideTip(true)
            if (options.positiveTip) options.positiveTip.hideTip(true)
            if (options.tipsDisable)
              options.tipsDisable.forEach(function(tip) {
                tip.disabled(false)
              })
            ...
            if (_sourceDown && _targetValid) {
              ...
              var source = _sourceDown,
                target = _targetMove
              finishPromise.then(function(ok) {
                if (ok) create_edge(diagram, source, target)
              })
            } else if (_sourceDown) {
              if (_mode.conduct().cancelDragEdge) _mode.conduct().cancelDragEdge(_sourceDown)
            }
        ...
        diagram
          ...
          .on('mousemove.draw-graphs', function() {
            ...
            if (_sourceDown) {
              ...
              _crossout = null
              if (check_invalid_drag(coords)) return
              if (_mode.conduct().dragCanvas) _mode.conduct().dragCanvas(_sourceDown, coords)
              if (_mode.conduct().changeDragTarget && _targetMove) _mode.conduct().changeDragTarget(_sourceDown, null)
              _targetMove = null
              _hintData[0].target = { x: coords[0], y: coords[1] }
              update_hint()
              update_crossout()
    ...
    dc_graph.match_ports = function(diagram, symbolPorts) {
      ...
      function is_valid(sourcePort, targetPort) {
        return (
          (_strategy.allowParallel() || !has_parallel(sourcePort, targetPort)) &&
          _strategy.isValid()(sourcePort, targetPort)
        )
      }
      function why_invalid(sourcePort, targetPort) {
        return (
          (!_strategy.allowParallel() &&
            has_parallel(sourcePort, targetPort) &&
      ...
      var _strategy = {
        ...
        hoverPort: function(port) {
          if (port) {
            ...
            if (_validTargets.length) return change_state(_validTargets, 'shimmer-medium')
          } else if (_validTargets) return change_state(_validTargets, 'small')
          return null
        },
        ...
        changeDragTarget: function(source, target) {
          var nids,
            valid = target && is_valid(source.port, target.port),
            before
          if (valid) {
    ...
    dc_graph.match_opposites = function(diagram, deleteProps, options) {
      ...
      var _strategy = {
        ...
        dragCanvas: function(source, coords) {
          ...
          var cpos = port_pos(closest[0].port),
            spos = port_pos(source.port)
        ...
        changeDragTarget: function(source, target) {
          ...
          if (valid) {
            ...
            reset_deletables(
              null,
              _validTargets.filter(function(p) {
        ...
        finishDragEdge: function(source, target) {
          if (is_valid(source.port, target.port)) {
            ...
            if (options.delete_edges) {
              var edgeKeys = source.port.edges
                .map(diagram.edgeKey.eval)
                .concat(target.port.edges.map(diagram.edgeKey.eval))
    ...
    dc_graph.wildcard_ports = function(options) {
      ...
      return {
        isValid: function(p1, p2) {
          return (
            (get_type(p1) === null) ^ (get_type(p2) === null) ||
            (get_type(p1) !== null && get_type(p1) === get_type(p2))
          )
        },
        whyInvalid: function(p1, p2) {
          return (
            (get_type(p1) === null && get_type(p2) === null && "can't connect wildcard to wildcard") ||
    ...
    dc_graph.symbol_port_style = function() {
      ...
      function symbol_fill(p) {
        ...
        return symcolor ? (_style.colorScale() ? _style.colorScale()(symcolor) : symcolor) : 'none'
      }
      ...
      function port_symbol(p) {
        if (!_style.symbolScale()) _style.symbolScale(d3.scale.ordinal().range(d3.shuffle(_style.content().enum())))
      ...
      function shimmer_radius(p) {
        return /-medium$/.test(p.state) ? _style.mediumRadius.eval(p) : _style.largeRadius.eval(p)
      }
      ...
      _style.animateNodes = function(nids, before) {
        ...
        var node = _node.filter(function(n) {
        ...
        var shimmer = symbol.filter(function(p) {
            return /^shimmer/.test(p.state)
          }),
          nonshimmer = symbol.filter(function(p) {
            return !/^shimmer/.test(p.state)
          })
        if (shimmer.size()) {
          if (before) before.each('end', repeat)
          else repeat()
        }
        ...
        function repeat() {
          var shimin = shimmer
            ...
            .duration(1000)
            .ease('bounce')
          shimin.selectAll('.port-outline').call(
            _style.outline().draw(function(p) {
              return shimmer_radius(p) + _style.portPadding.eval(p)
            }),
          )
          shimin.selectAll('.port-symbol').call(_style.content().draw(port_symbol, shimmer_radius))
          var shimout = shimin
            .transition()
            .duration(1000)
            .ease('sin')
          shimout.selectAll('.port-outline').call(
            _style.outline().draw(function(p) {
              return _style.smallRadius.eval(p) + _style.portPadding.eval(p)
            }),
          )
          shimout.selectAll('.port-symbol').call(_style.content().draw(port_symbol, _style.smallRadius.eval))
          shimout.each('end', repeat)
        }

        var trans = nonshimmer.transition().duration(250)
        trans.selectAll('.port-outline').call(
          _style.outline().draw(function(p) {
            return hover_radius(p) + _style.portPadding.eval(p)
          }),
        )
        trans.selectAll('.port-symbol').call(_style.content().draw(port_symbol, hover_radius))

        function text_showing(p) {
          return p.state === 'large' || p.state === 'medium'
        }
        trans.selectAll('text.port-label').attr({
          opacity: function(p) {
        ...
        trans.selectAll('rect.port-label-background').attr('opacity', function(p) {
          return text_showing(p) ? 1 : 0
        })
      ...
      _style.eventPort = function() {
        ...
        if (d3.event.target.parentNode.tagName === 'g' && parent.classed('port')) return parent.datum()
        return null
      }
      _style.drawPorts = function(ports, nodePorts, node) {
        ...
        outline.exit().remove()
        var outlineEnter = outline
          .enter()
          .append(_style.outline().tag())
          .attr({
            class: 'port-outline',
            fill: outline_fill,
        ...
        if (_style.outline().init) outlineEnter.call(_style.outline().init)
        outlineEnter.call(
          _style.outline().draw(function(p) {
            return _style.smallRadius.eval(p) + _style.portPadding.eval(p)
          }),
        ...
        var symbolEnter = portEnter
          .append(_style.content().tag())
        ...
        labelEnter.append('rect').attr({
          class: 'port-label-background',
          'pointer-events': 'none',
        })
        labelEnter.append('text').attr({
          class: 'port-label',
        ...
        port.selectAll('rect.port-label-background').attr({
          x: function(p) {
      ...
      _style.enableHover = function(whether) {
        if (!_drawConduct) {
          if (_style.parent()) {
            ...
            if (draw) _drawConduct = draw.conduct()
          }
        }
    ...
    dc_graph.symbol_port_style.content.letter = function() {
      var _symbol = {
        ...
        draw: function(symf, rf) {
          return function(symbols) {
            symbols.text(symf).attr({
              'dominant-baseline': 'middle',
              'text-anchor': 'middle',
            })
            symbols.each(function(p) {
              if (!p.symbol_size) p.symbol_size = getBBoxNoThrow(this)
            })
            symbols.attr('transform', function(p) {
              return 'scale(' + (2 * rf(p)) / p.symbol_size.height + ') translate(' + [0, 2].join(',') + ')'
            })
          }
        },
      }
    ...
    function process_dot(callback, error, text) {
      ...
      if (graphlibDot.parse) {
        // graphlib-dot 1.1.0 (where did i get it from?)
    ...
    function process_dsv(callback, error, data) {
      ...
      nodes = nodes.values().map(function(k) {
        return { name: k }
      })
      callback(null, {
        nodes: nodes,
    ...
    dc_graph.match_mime_type = function(mime) {
      return dc_graph.file_formats.find(function(format) {
        ...
        if (!Array.isArray(mimes)) mimes = [mimes]
    ...
    function unknown_format_error(filename) {
      ...
      if (spl.length) return new Error('do not know how to process graph file extension ' + spl[spl.length - 1])
      else return new Error('need file extension to process graph file automatically, filename ' + filename)
    }
      ...
      if (file2) {
        ...
        queue()
          ...
          .await(function(error, nodes, edges) {
            ...
            else callback(null, { nodes: nodes.results, edges: edges.results })
          })
      } else {
        var format
        if (/^data:/.test(file1)) {
    ...
    dc_graph.load_graph_text = function(text, filename, callback) {
      ...
      if (format) format.from_text(text, callback)
    ...
    dc_graph.munge_graph = function(data, nodekeyattr, sourceattr, targetattr) {
      ...
      if (!can_get_graph_from_this(data)) {
        ...
        var wi = wrappers.findIndex(function(f) {
          return data[f] && can_get_graph_from_this(data[f])
        })
        if (wi < 0) throw new Error("couldn't find the data!")
      ...
      if (edge0[sa] === undefined) {
        ...
        if (edge0.node0 && edge0.node1) {
          // specific conflict here
          sa = 'node0'
          ta = 'node1'
        } else {
          ...
          if (!candidates.length) {
            ...
            if (wi < 0) {
              if (sourceattr) throw new Error('sourceattr ' + sa + " didn't work")
              else throw new Error("didn't find any source attr")
            }
            edges = edges.map(function(e) {
              return e[edgewrappers[wi]]
            })
            edge0 = edges[0]
            candidates = find_attr(edge0, sourceattrs)
          }
          if (candidates.length > 1) console.warn('found more than one possible source attr', candidates)
          sa = candidates[0]

          candidates = find_attr(edge0, targetattrs)
          if (!candidates.length) {
            if (targetattr && !edge0[targetattr]) throw new Error('targetattr ' + ta + " didn't work")
            else throw new Error("didn't find any target attr")
          }
          if (candidates.length > 1) console.warn('found more than one possible target attr', candidates)
          ta = candidates[0]
      ...
      if (node0[nka] === undefined) {
        ...
        if (!candidates.length) {
          ...
          if (wi < 0) {
            if (nodekeyattr) throw new Error('nodekeyattr ' + nka + " didn't work")
            else throw new Error("couldn't find the node data")
          }
          nodes = nodes.map(function(n) {
            return n[nodewrappers[wi]]
          })
          node0 = nodes[0]
          candidates = find_attr(node0, nodekeyattrs)
        }
        if (candidates.length > 1) console.warn('found more than one possible node key attr', candidates)
        nka = candidates[0]
      }
    ...
    dc_graph.flat_group = (function() {
      ...
      function dim_group(ndx, id_accessor) {
        ...
        return {
          crossfilter: ndx,
          dimension: dimension,
          group: non_null(dimension.group().reduce(reduce_01.add, reduce_01.remove, reduce_01.init)),
        }
      }
      ...
      return {
        ...
        make: function(source, id_accessor) {
          var cf
          if (Array.isArray(source)) cf = crossfilter(source)
          else cf = source
    ...
    var convert_tree_helper = function(data, attrs, options, parent, level, inherit) {
      ...
      if (attrs.length > (options.valuesByAttr ? 1 : 0)) {
        ...
        var children = data.map(function(v) {
          ...
          if (childKey) {
            ...
            if (options.ancestorKeys) {
              inherit = inherit || {}
              if (attr) inherit[attr] = key
              node = Object.assign({}, inherit)
            } else node = {}
            node[options.nodeKey] = childKey
            if (options.label && options.labelFun) node[options.label] = options.labelFun(key, attr, v)
            if (options.level) node[options.level] = level + 1
            nodes.push(node)
            if (parent) {
              var edge = {}
          ...
          var recurse = convert_tree_helper(
            children,
            attrs.slice(0),
            options,
            childKey,
        ...
        return {
          nodes: Array.prototype.concat.apply(nodes, children.map(dc.pluck('nodes'))),
          edges: Array.prototype.concat.apply(edges, children.map(dc.pluck('edges'))),
        }
      } else
        return {
          nodes: data.map(function(v) {
            v = Object.assign({}, v)
            if (options.level) v[options.level] = level + 1
            return v
          }),
    ...
    dc_graph.convert_tree = function(data, attrs, options) {
      ...
      if (Array.isArray(data)) return convert_tree_helper(data, attrs, options, options.root, 0, options.inherit)
      else {
        attrs = [''].concat(attrs)
      ...
      return {
        all: function() {
          ...
          group.all().forEach(function(kv) {
            ...
            var min = dir ? source : target,
              max = dir ? target : source
            ST[min] = ST[min] || {}
      ...
      var reader = {
        ...
        data: function(data) {
          ...
          reader.pathList.eval(data).forEach(function(path) {
            if ((path._range = reader.timeRange.eval(path))) {
              // ugh modifying user data
              if (has_ranges === false) throw new Error("can't have a mix of ranged and non-ranged paths")
              has_ranges = true
            } else {
              if (has_ranges === true) throw new Error("can't have a mix of ranged and non-ranged paths")
              has_ranges = false
    ...
    dc_graph.path_selector = function(parent, reader, pathsgroup, chartgroup) {
      ...
      function draw_paths(diagram, paths) {
        ...
        pathlist
          ...
          .attr('transform', function(path, i) {
            return 'translate(0, ' + i * 20 + ')'
          })
          .each(function(path_data, i) {
            var nodes = path_data.element_list.filter(function(d) {
              return d.element_type === 'node'
            })
            // line
            var line = d3.select(this).append('line')
            line
              .attr('x1', xpadding + space)
              .attr('y1', radius + 1)
      ...
      function draw_hovered() {
        ...
        root.selectAll('g.path-selector').each(function(d, i) {
    ...
    dc_graph.node_name = function(i) {
      ...
      else if (i < 52) return String.fromCharCode(65 + i - 26)
      else if (i < 52 * 52) return dc_graph.node_name(Math.floor(i / 52)) + dc_graph.node_name(i % 52)
      ...
      switch (type) {
        ...
        case 'cliquestf':
          ...
          for (i = 0; i < N; ++i) {
            ...
            for (j = 0; j < i; ++j) edges.push(dc_graph.edge_object(namef, i, j, { notLayout: true, undirected: true }))
          }
          if (type === 'cliquestf')
            for (i = 0; i < N; ++i) {
        ...
        case 'wheel':
          nodes = new Array(N)
          for (i = 0; i < N; ++i) nodes[i] = dc_graph.node_object(i, { name: nodePrefix + dc_graph.node_name(i) })
    ...
    dc_graph.wheel_edges = function(namef, nindices, R) {
      ...
      for (i = 0; i < N / 2; ++i) {
        ...
        if (N % 2 && i != Math.floor(N / 2))
          edges.push(
            dc_graph.edge_object(namef, nindices[i], nindices[(i + N - strutSkip) % N], { distance: strutLength }),
          )
      }
      return edges
    }
    ...
    dc_graph.random_graph = function(options) {
      options = Object.assign(
        {
          ...
          colorTag: 'color',
          dashTag: 'dash',
          nodeKeyGen: function(i) {
            return 'n' + i
          },
          edgeKeyGen: function(i) {
            return 'e' + i
          },
      ...
      if (isNaN(options.newNodeProb)) options.newNodeProb = 0.9
      if (options.newNodProb <= 0) options.newNodeProb = 0.1
      var _nodes = [],
        _edges = []
      function new_node() {
        var n = {}
      ...
      return {
        ...
        generate: function(N) {
          while (N-- > 0) {
            ...
            var n1, n2
            if (!_nodes.length || choice < options.newComponentProb) n1 = new_node()
            else n1 = random_node()
            if (choice < options.newNodeProb) n2 = new_node()
            else n2 = random_node()
            if (n1 && n2) {
              var edge = {}
              ...
              if (options.log) console.log(n1[options.nodeKey] + ' -> ' + n2[options.nodeKey])
              _edges.push(edge)
            }
          }
        },
        remove: function(N) {
          while (N-- > 0) {
            var choice = Math.random()
            if (choice < options.removeEdgeProb) _edges.splice(Math.floor(Math.random() * _edges.length), 1)
            else {
              var n = _nodes[Math.floor(Math.random() * _nodes.length)]
              var eis = []
              _edges.forEach(function(e, ei) {
                if (e[options.sourceKey] === n[options.nodeKey] || e[options.targetKey] === n[options.nodeKey])
                  eis.push(ei)
              })
    ...
    var dont_use_key = deprecation_warning(
      'dc_graph.line_breaks now takes a string - d.key behavior is deprecated and will be removed in a later version',
    )
    ...
    dc_graph.line_breaks = function(charexp, max_line_length) {
      ...
      return function(s) {
        ...
        do {
          ...
          if (result) part = s.slice(i, regexp.lastIndex)
          else part = s.slice(i)
    ...
    dc_graph.build_type_graph = function(nodes, edges, nkey, ntype, esource, etarget) {
      ...
      nodes.forEach(function(n) {
        nmap[nkey(n)] = n
        var t = ntype(n)
        if (!tnodes[t]) tnodes[t] = { type: t }
      })
      edges.forEach(function(e) {
        var source = esource(e),
          target = etarget(e),
          sn,
          tn
        if (!(sn = nmap[source])) throw new Error('source key ' + source + ' not found!')
        if (!(tn = nmap[target])) throw new Error('target key ' + target + ' not found!')
      ...
      return {
        nodes: Object.keys(tnodes).map(function(k) {
          return tnodes[k]
        }),
        edges: Object.keys(tedges).map(function(k) {
          return tedges[k]
        }),
      }
    }
