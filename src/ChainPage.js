  ...
  renderGraph() {
    ...
    var options = {
      ...
      gvattr: {
        ...
        exert: function(val, diagram) {
          if (val) dc_graph.apply_graphviz_accessors(simpleDiagram);
          else {
            simpleDiagram
    ...
    var url_output = sync_url.output(),
      more_output;
    sync_url.output(function(params) {
      url_output(params);
      if (more_output) more_output(params);
    });
    ...
    function on_load(filename, error, data) {
      ...
      function update_data_link() {
        d3.select("#data-link").attr(
          "href",
          sync_url.what_if_url({
            file: dc_graph.data_url({ nodes: nodes, edges: edges })
          })
        );
      }
      ...
      var edge_flat = dc_graph.flat_group.make(edges, edge_key),
        ...
        cluster_flat = dc_graph.flat_group.make(data.clusters || [], function(
          d
        ) {
          return d.key;
        });
      ...
      simpleDiagram
        ...
        .autoZoom("once")
        .restrictPan(true)
        .nodeDimension(node_flat.dimension)
        .nodeGroup(node_flat.group)
        .edgeDimension(edge_flat.dimension)
        ...
        .clusterDimension(cluster_flat.dimension)
        .clusterGroup(cluster_flat.group)
        .nodeParentCluster(
          data.node_cluster
            ? function(n) {
                return data.node_cluster[n.key];
              }
            : null
        )
        .clusterParent(function(c) {
          return c.parent;
        })
        // aesthetics
        .nodeTitle(null); // deactivate basic tooltips
      ...
      if (sync_url.vals.neighbors) {
        var highlight_neighbors = dc_graph
          .highlight_neighbors({
            edgeStroke: "orangered",
            edgeStrokeWidth: 3
          })
          .durationOverride(0);
        simpleDiagram.child("highlight-neighbors", highlight_neighbors);
      }
